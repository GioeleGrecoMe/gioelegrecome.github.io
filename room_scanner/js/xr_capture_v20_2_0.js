...
export class XRCaptureController {
  ...
  _sampleHitTest(frame,time,motion){if(!this.hitTestSource||time-(this.lastHitTime||0)<420)return;this.lastHitTime=time;try{const hit=frame.getHitTestResults(this.hitTestSource)?.[0];const pose=hit?.getPose(this.refSpace);if(!pose)return;const m=pose.transform.matrix,p=[m[12],m[13],m[14]],n=norm3([m[4],m[5],m[6]]),toCam=sub3(motion.position,p),nn=(n[0]*toCam[0]+n[1]*toCam[1]+n[2]*toCam[2])<0?scale3(n,-1):n;const a=new Float32Array([p[0],p[1],p[2],nn[0],nn[1],nn[2],145,145,145,.36]);this.hitChunk.push({segmentId:this.segmentId,timeXR:time,timeEpoch:Date.now(),position:p,normal:nn,cameraPosition:motion.position});if(this.hitChunk.length>=24)this._flushHitChunk().catch(e=>this.diag.error('hit-test-write-failed',e));this.mapWorker?.postMessage({type:'depthBatch',buffer:a.buffer,viewOrigin:motion.position,viewKey:quantizedViewKey(motion.position),source:'hit-test',time:Date.now()},[a.buffer]);}catch(e){if(!this._hitErrorLogged){this._hitErrorLogged=true;this.diag.error('hit-test-frame-failed',e);}}
  }
  _shouldTakePhoto(time,guidance){if(this.repo.pendingCount()>=this.profile.maxPendingWrites)return false;if(this.forcePhoto)return time-this.lastPhoto>320;if(time-this.lastPhoto<this.profile.minPhotoIntervalMs)return false;const needsDeep=!!(guidance&&this.gridTiles.find(t=>t.id===guidance.tileId)?.needDeep&&this.motion.stability>.12);const periodic=time-this.lastPhoto>(this.profile.denseRgbIntervalMs||1250)&&this.motion.linearSpeed<1.15&&this.motion.angularSpeed<2.2;return needsDeep||periodic;}
  async _capturePhotoInFrame(view,time,motion,guidance){
    ...
    if(reason==='markpoint'||this.markpointRequest){this.markpointRequest=false;const candidate=buildMarkpointCandidate(capture,this.lastDepthCandidate,meta);this.cb.onMarkpointCandidate?.(candidate);}
