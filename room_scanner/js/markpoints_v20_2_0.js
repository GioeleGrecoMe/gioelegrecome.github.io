...
export class MarkpointManager {
  ...
  async accept(candidate,validation){
    ...
    this.points.push(point);await this.repo.enqueueRecord(this.sessionId,'markpoint',point,{key:`${this.sessionId}/markpoint/${point.id}`});this.worker?.postMessage({type:'markpoint',markpointId:point.id,position:point.position});await this.diag?.log('markpoint-created',{id:point.id,quality:point.quality,valid:validation.valid,reasons:validation.reasons});return point;
  }
  async observeFrame({frameId,cameraPosition,projectedPatches=[]}){
    ...
    for(const p of updates)await this.repo.enqueueRecord(this.sessionId,'markpoint-update',{id:p.id,status:p.status,confirmations:p.confirmations},{key:`${this.sessionId}/markpoint-update/${p.id}/${Date.now()}`});return updates;
  }
}
