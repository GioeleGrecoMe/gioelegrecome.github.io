(() => {
  ...
  // D12_XR_TRACE: loaded before the V10 application scripts. This file is diagnostics-only:
  // it records WebXR lifecycle and user-activation state without changing XR options or rendering.
  const BUILD = 'room-scanner-v11-d12-xrtrace-2026-08-16';
  const STORE = 'roomScannerV11D12XRTrace';
  const events = [];
  ...
  function stackText() {
    try { return String(new Error('D12 XR call stack').stack || '').split('\n').slice(1, 8).join('\n'); }
    catch (_) { return ''; }
  }
  ...
  function push(type, detail = {}) {
    ...
    try {
      window.__DEPTH_V11_D12_PUSH__ && window.__DEPTH_V11_D12_PUSH__('XR ' + type, item.detail);
    } catch (_) {}
  }
  ...
  function installXRHook() {
    ...
    if (xr.requestSession && xr.requestSession.__roomScannerD12Wrapped) {
    ...
    try { Object.defineProperty(wrapped, '__roomScannerD12Wrapped', { value: true }); } catch (_) {}
  ...
  window.__ROOM_SCANNER_D12_XR_REPORT__ = () => ({
    schema: 'room-scanner-v11-d12-xrtrace',
    build: BUILD,
