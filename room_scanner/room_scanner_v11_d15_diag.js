(() => {
  'use strict';
  const BUILD='room-scanner-v11-d15-depth-boottrace-2026-08-16';
  const pageEvents=[];
  const MAX_EVENTS=80;
  const RX=/depth|onnx|ort\.|wasm|webgpu|inference|session/i;
  let previousTrace=null;try{const raw=localStorage.getItem('roomScannerV11D15LastTrace');if(raw)previousTrace=JSON.parse(raw)}catch(_){}
  function push(type,detail){
    ...
    try{localStorage.setItem('roomScannerV11D15LastTrace',JSON.stringify({build:BUILD,updatedAt:new Date().toISOString(),href:location.href,pageEvents:pageEvents.slice(-80)}))}catch(_){}
    updateCompactStatus();
    updateLiveTrace();
  }
  window.__DEPTH_V11_D15_PUSH__=(type,detail)=>push(type,detail);
  ...
  function report(){
    return {
      ...
      xrTrace:(()=>{try{return window.__ROOM_SCANNER_D15_XR_REPORT__?window.__ROOM_SCANNER_D15_XR_REPORT__():null}catch(e){return {error:String(e&&e.stack||e)}}})(),
      d15SourcePatch:{depthGate:window.__ROOM_SCANNER_D15_DEPTH_GATE_STATE__||null,depthControl:(()=>{try{return window.__ROOM_SCANNER_D15_GET_DEPTH_STATUS__?window.__ROOM_SCANNER_D15_GET_DEPTH_STATUS__():null}catch(e){return {error:String(e&&e.stack||e)}}})(),photoGuardCount:window.__ROOM_SCANNER_D15_PHOTO_GUARD_COUNT__||0},
      previousTrace,
  ...
  function updateLiveTrace(){
    ...
    const lines=['ROOM SCANNER V11-D15  |  '+BUILD];
    try{const xr=window.__ROOM_SCANNER_D15_XR_REPORT__&&window.__ROOM_SCANNER_D15_XR_REPORT__();for(const x of (xr&&xr.events||[]).slice(-8))lines.push('XR '+compactEventLine(x))}catch(_){}
  ...
  function ensurePanel(){
    ...
    panel.id='depthV11D15Diag';
    badge=document.getElementById('v11D15StaticBanner');
    ...
    panel.querySelector('#depthV11Prev').onclick=()=>{ta.value=previousTrace?JSON.stringify(previousTrace,null,2):'Nessun log D15 precedente salvato';ta.style.display='block'};
