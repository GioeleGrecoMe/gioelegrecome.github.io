(() => {
  'use strict';
  const BUILD='room-scanner-v11-d6-depth-boottrace-2026-08-16';
  const pageEvents=[];
  const MAX_EVENTS=80;
  const RX=/depth|onnx|ort\.|wasm|webgpu|inference|session/i;
  let previousTrace=null;try{const raw=localStorage.getItem('roomScannerV11D6LastTrace');if(raw)previousTrace=JSON.parse(raw)}catch(_){}
  function push(type,detail){
    ...
    try{localStorage.setItem('roomScannerV11D6LastTrace',JSON.stringify({build:BUILD,updatedAt:new Date().toISOString(),href:location.href,pageEvents:pageEvents.slice(-80)}))}catch(_){}
    updateCompactStatus();
    updateLiveTrace();
  }
  window.__DEPTH_V11_D6_PUSH__=(type,detail)=>push(type,detail);
  ...
  function updateLiveTrace(){
    ...
    const lines=['ROOM SCANNER V11-D6  |  '+BUILD];
  ...
  function ensurePanel(){
    ...
    panel.id='depthV11D6Diag';
    badge=document.getElementById('v11D6StaticBanner');
    ...
    panel.querySelector('#depthV11Wasm').onclick=()=>{try{sessionStorage.setItem('roomScannerV11D6ForceWasm','1')}catch(_){};try{if(typeof S!=='undefined'&&S&&S.depthAI)S.depthAI.forceWasm=true}catch(_){};push('manual-force-wasm',{applied:true});status.textContent='WASM forzato per la prossima inizializzazione Depth';};
    ...
    panel.querySelector('#depthV11Prev').onclick=()=>{ta.value=previousTrace?JSON.stringify(previousTrace,null,2):'Nessun log D6 precedente salvato';ta.style.display='block'};
