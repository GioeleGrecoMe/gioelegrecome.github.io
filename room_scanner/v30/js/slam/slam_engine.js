...
export class SlamEngine{
 constructor(frontend,{cameraHeightM=1.35,fovDeg=62,keyframeIntervalMs=900}={}){this.frontend=frontend;this.cameraHeightM=cameraHeightM;this.fovDeg=fovDeg;this.keyframeIntervalMs=keyframeIntervalMs;this.reset();}
 ...
 integrateDepth(kf,result){
  ...
  if(cal.confidence>.25||!this.metricCalibration)this.metricCalibration={mode:cal.mode,a:cal.a,b:cal.b,confidence:cal.confidence,source:cal.source};kf.depth={width,height,data:depth};kf.depthCalibration=cal;
  ...
  let added=0;for(let i=0;i<kf.trackIds.length;i++){const u=kf.featureX[i]/kf.analysisWidth*width,v=kf.featureY[i]/kf.analysisHeight*height,x=Math.max(0,Math.min(width-1,Math.round(u))),y=Math.max(0,Math.min(height-1,Math.round(v))),raw=depth[y*width+x],z=metricDepth(raw,cal);if(!Number.isFinite(z)||z<.15||z>12)continue;const Kd={fx:kf.K.fx*width/kf.analysisWidth,fy:kf.K.fy*height/kf.analysisHeight,cx:width/2,cy:height/2,width,height},pc=backproject(u,v,z,Kd),pw=cameraToWorld(kf.pose,pc),id=kf.trackIds[i],old=this.landmarks.get(id);if(old){old.p=[old.p[0]*.7+pw[0]*.3,old.p[1]*.7+pw[1]*.3,old.p[2]*.7+pw[2]*.3];old.views++;}else{this.landmarks.set(id,{id,p:pw,views:1,firstKf:kf.id});added++;}}
  return {calibration:cal,landmarksAdded:added,totalLandmarks:this.landmarks.size};
 }
