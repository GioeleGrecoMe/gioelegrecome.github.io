...
export class SlamEngine{
 ...
 _integrateVisualKeyframe(kf,motionPrior){
  ...
  const dx=median(flows.map(p=>p[0])),dy=median(flows.map(p=>p[1])),flow=Math.hypot(dx,dy),imu=motionPrior||{},base=imu.translationL||0;if(base<.018&&(flow<7||(imu.rotationRad||0)>.16))return {ok:false,reason:'need-lateral-motion',added:0,matches:flows.length};
  const fallback=base>=.018?base:Math.min(.075,.025+flow*.003),dir0=[-dx/Math.max(1,kf.K.fx),dy/Math.max(1,kf.K.fy),.16],n0=Math.hypot(...dir0)||1,dir=dir0.map(v=>v/n0),delta=qRotate(prev.pose.q,dir.map(v=>v*fallback));kf.pose={p:[prev.pose.p[0]+delta[0],prev.pose.p[1]+delta[1],prev.pose.p[2]+delta[2]],q:[...kf.pose.q]};this.pose=poseClone(kf.pose);
  let added=0;for(let i=0;i<kf.trackIds.length;i++){const id=kf.trackIds[i],j=old.get(id);if(j===undefined)continue;const a=this._ray(prev,prev.featureX[j],prev.featureY[j]),b=this._ray(kf,kf.featureX[i],kf.featureY[i]),p=this._triangulate(a,b);if(!p)continue;const oldLm=this.landmarks.get(id),conf=Math.min(1,.18+Math.min(.45,flow/28)+Math.min(.35,(imu.confidence||0)*.5));if(oldLm){oldLm.p=oldLm.p.map((v,k)=>v*.65+p[k]*.35);oldLm.views++;oldLm.confidence=Math.max(oldLm.confidence||0,conf);}else{this.landmarks.set(id,{id,p,views:1,confidence:conf,firstKf:kf.id});added++;}const lm=this.landmarks.get(id);kf.visualPoints.push({x:kf.featureX[i],y:kf.featureY[i],p:[...lm.p],confidence:lm.confidence||conf,views:lm.views});}
