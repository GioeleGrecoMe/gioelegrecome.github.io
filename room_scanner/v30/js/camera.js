 ...
 * V30.11.2 can adopt the already-open metric-bridge camera stream so metric
...
export class CameraController extends EventTarget{
  constructor({video,width=320,height=480,fps=12,log=null,stream=null}={}){
    ...
    this.canvas=document.createElement('canvas');this.canvas.width=width;this.canvas.height=height;this.ctx=this.canvas.getContext('2d',{willReadFrequently:true});this.running=false;this._timer=0;
  }
  adoptStream(stream){
