...
export class ImuTracker{
 constructor(){this.samples=[];this.latest=null;this.permission='unknown';this._motion=e=>this.onMotion(e);this._orientation=e=>this.onOrientation(e);this.orientation=null;this.lastPriorOrientation=null;}
 async start(){
 ...
 motionPrior(){const recent=this.samples.slice(-24);let a=[0,0,0];for(const s of recent)for(let k=0;k<3;k++)a[k]+=s.a[k]||0;if(recent.length)a=a.map(v=>v/recent.length);const magnitude=Math.hypot(...a),direction=magnitude>.08?a.map(v=>v/magnitude):[0,0,0],translationL=Math.max(0,Math.min(.14,(magnitude-.10)*.035)),o=this.orientation;let rotationRad=0;if(o&&this.lastPriorOrientation){const diff=(x,y)=>{let d=(x??0)-(y??0);while(d>180)d-=360;while(d<-180)d+=360;return d;};rotationRad=Math.hypot(diff(o.alpha,this.lastPriorOrientation.alpha),diff(o.beta,this.lastPriorOrientation.beta),diff(o.gamma,this.lastPriorOrientation.gamma))*Math.PI/180;}if(o)this.lastPriorOrientation={...o};return {translationL,confidence:Math.max(0,Math.min(1,(magnitude-.10)/2.2)),direction,rotationRad};}
