...
export async function runSelfTests(log){
  ...
  await run('wasm-portrait-frame',async()=>{const f=new WasmVisionFrontend(CONFIG.wasmCore);await f.init();const w=270,h=480,img=new Uint8Array(w*h);for(let y=0;y<h;y++)for(let x=0;x<w;x++)img[y*w+x]=((x*11+y*5)^((x>>4)*31)^((y>>4)*17))&255;const r=f.process(img,w,h,{maxFeatures:500,threshold:12});if(r.count<8)throw new Error(`portrait frame weak output ${r.count}`);return {width:w,height:h,features:r.count,limits:f.limits};});
