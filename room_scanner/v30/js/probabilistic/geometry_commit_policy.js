...
export function evaluateFinalGeometryPolicy({meshQuality=null,rawMeshQuality=null,meshCleanup=null,surfaceConsensus=null,gaussianCount=0,frames=[],sparseDepthEnvelope=null,mvsValidation=null,depthGeometryPolicy=null}={}){
  ...
  // A globally-consistent splat map and a watertight/coherent TSDF mesh are
  // different deliverables.  Fragmented topology must withhold the mesh, but it
  // must not erase an already authoritative multi-view splat consensus.
  const splatCommitReady=gaussianCount>0&&denseAuthorityReady&&consensusReady&&!scaleExplosion;
  const meshCommitReady=splatCommitReady&&faces>0&&!rawFragmentationHigh&&topologyCoherent;
  const commitReady=meshCommitReady; // backwards-compatible: historically this meant mesh+surface.
