...
export class SingleOptimizerRuntime{
  ...
  async rebuildAccepted(graph,options={}){
    ...
    try{
      ...
      const splatReady=geometryPolicy?.splatCommitReady??geometryPolicy?.commitReady??true,meshReady=geometryPolicy?.meshCommitReady??geometryPolicy?.commitReady??true,geometryWithheldReason=geometryPolicy&&splatReady===false?(geometryPolicy.reason||'final-geometry-policy-rejected'):null,meshWithheldReason=splatReady&&meshReady===false?(geometryPolicy?.reason||'mesh-policy-rejected'):null,withheldReason=rgbWithheldReason||geometryWithheldReason;
      if(withheldReason){this.trace('commit-withheld',{elapsedMs,reason:withheldReason,rgbWithheldReason,geometryWithheldReason,meshWithheldReason,reconciled:useReconciled,geometryPolicy,poseScaffoldPolicy:map?.stats?.poseScaffoldPolicy||null,depthGeometryPolicy:map?.stats?.depthGeometryPolicy||null,mvsValidation:map?.stats?.mvsValidation||null});return {map,stats,snapshot,elapsedMs,reconciled:useReconciled,reconcileGate,acceptedFrameIds:[...acceptedFrameIds],withheldReason,meshWithheldReason};}
      if(meshWithheldReason)this.trace('mesh-commit-withheld',{elapsedMs,reason:meshWithheldReason,reconciled:useReconciled,geometryPolicy});
      return {map,stats,snapshot,elapsedMs,reconciled:useReconciled,reconcileGate,acceptedFrameIds:[...acceptedFrameIds],meshWithheldReason};
  ...
  async makePreview(opt,job,summary,traceBase){
    ...
    if(job.previewMap&&String(opt.lastFeedbackPhase||opt.computeStats()?.feedbackPhase)==='depth-feedback'&&(summary.deepFrames||0)>0){
      const mt=now();try{
        ...
        previewStats=map.stats||null;const previewPolicy=map.stats?.geometryPolicy||null,previewSplatReady=previewPolicy?.splatCommitReady??previewPolicy?.commitReady??true;previewGaussians=previewSplatReady===false?null:(map.gaussians||[]).slice(0,Number(job.previewMaxSurfels)||2200);if(!previewGaussians?.length&&map.gaussians?.length)this.trace('preview-map-withheld',{...traceBase,reason:map.stats?.geometryPolicy?.reason||'geometry-policy',geometryPolicy:map.stats?.geometryPolicy||null,poseScaffoldPolicy:map.stats?.poseScaffoldPolicy||null,mvsValidation:map.stats?.mvsValidation||null,depthGeometryPolicy:map.stats?.depthGeometryPolicy||null});
