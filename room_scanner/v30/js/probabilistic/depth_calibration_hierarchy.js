...
import {assessDepthCalibrationObservability} from './depth_observability.js?v=30.40.0';
 ...
 */
export function solveHierarchicalDepthCalibration({frames=[],landmarks=[],deepFactors=[],previous=null,frameReliability=null,iterations=8,minGlobalFrames=5,minGlobalAnchors=40,temporalSigmaA=.10,temporalSigmaB=.035}={}){
  ...
  const domain=normalizationDomain(allRaw,previous?.domain),states=new Array(frames.length).fill(null),prevMap=new Map((previous?.frames||[]).map(x=>[String(x.frameId),x]));
  ...
  for(let i=0;i<frames.length;i++){const old=prevMap.get(String(frames[i]?.frameId)),pred=temporalPrediction(i,states,prevMap,frames,seed);states[i]={frameId:String(frames[i]?.frameId),a:positive(old?.a??pred.a),b:Number.isFinite(+old?.b)?+old.b:pred.b,mode:observability[i].mode,observability:observability[i],confidence:.02,residualSigma:.15,anchorCount:normalized[i].length,inheritedFrom:pred.from||null};}
  ...
  for(let i=0;i<states.length;i++){
    const s=states[i],pairs=normalized[i],errs=[];let sw=0,se=0;for(const p of pairs){const r=s.a*globalResponse(p.x,gamma)+s.b-p.rho,w=p.w*cauchyWeight(r/Math.max(p.sigmaRho,.002),2.6);if(Number.isFinite(r)){errs.push(Math.abs(r));sw+=w;se+=w*r*r;}}
    const med=median(errs),mad=median(errs.map(e=>Math.abs(e-med))),sig=Math.max(.002,1.4826*(Number.isFinite(mad)?mad:.08));s.residualSigma=clamp(sig,.002,.35);s.confidence=calibrationConfidence(s,pairs);s.gamma=gamma.slice();s.domain={...domain};s.observability={...s.observability};if(s.mode==='full')full++;else if(s.mode==='shift-only')shift++;else inherited++;if(sw>0){totalResidual+=se;totalW+=sw;}
  }
  ...
  return {format:'ROOMSCAN-DEPTH-CAL-HIER-1',representation:'inverse-depth',domain,gamma,globalNonlinearityReady,frames:states,frameMap:new Map(states.map((s,i)=>[String(s.frameId),i])),stats:{frames:states.length,full,shiftOnly:shift,inherited,anchors:normalized.reduce((n,a)=>n+a.length,0),informativeFrames,informativeAnchors,globalNonlinearityReady,rmsInverseDepth:totalW>0?Math.sqrt(totalResidual/totalW):Infinity,minDerivative:minResponseDerivative(gamma)}};
}
...
export function serializeHierarchicalDepthCalibration(m){if(!m)return null;return {format:m.format,representation:m.representation,domain:m.domain,gamma:Array.from(m.gamma||[]),globalNonlinearityReady:!!m.globalNonlinearityReady,stats:m.stats,frames:(m.frames||[]).map(s=>({frameId:s.frameId,a:s.a,b:s.b,mode:s.mode,confidence:s.confidence,residualSigma:s.residualSigma,anchorCount:s.anchorCount,inheritedFrom:s.inheritedFrom,observability:s.observability}))};}
...
function calibrationConfidence(s,pairs){const o=s.observability?.score||0,data=1-Math.exp(-(pairs?.length||0)/10),res=Math.exp(-Math.min(8,s.residualSigma/.035));const freedom=s.mode==='full'?1:(s.mode==='shift-only'?.68:.32);return clamp((.08+.92*o)*(.18+.82*data)*(.25+.75*res)*freedom,.005,.995);}
function normalizationDomain(raw,old){const a=(raw||[]).filter(Number.isFinite).sort((x,y)=>x-y);if(a.length>=8){const lo=quantileSorted(a,.04),hi=quantileSorted(a,.96),center=(lo+hi)/2,scale=Math.max(EPS,(hi-lo)/2);if(old?.scale>0){return {center:mix(old.center,center,.25),scale:mix(old.scale,scale,.25),lo,hi};}return {center,scale,lo,hi};}return old?.scale>0?{...old}:{center:0,scale:1,lo:-1,hi:1};}
