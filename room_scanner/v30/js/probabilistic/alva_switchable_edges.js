...
export class SwitchableAlvaEdgeModel{
  ...
  importFactors(factors){this.edges=[];this.byFrame=new Map();for(const x of factors||[]){if(!x?.relativePose?.p||!x?.relativePose?.q)continue;const prior=clamp(Number(x.priorConfidence??x.prior??.5),this.minPrior,.995),e={aId:String(x.aId),bId:String(x.bId),observed:{p:x.relativePose.p.slice(0,3).map(Number),q:qNormalize(x.relativePose.q.slice(0,4).map(Number))},prior,switch:clamp(Number(x.switch??prior),.003,.998),translationSwitch:clamp(Number(x.translationSwitch??prior),.003,.998),rotationSwitch:clamp(Number(x.rotationSwitch??prior),.003,.998)};this.edges.push(e);for(const id of [e.aId,e.bId]){const q=this.byFrame.get(id)||[];q.push(e);this.byFrame.set(id,q);}}return this;}
  ...
  update(frames,{rgbFrameSupport=null,poseImprovement=null}={}){const fm=new Map((frames||[]).map(f=>[String(f.frameId),f]));for(const e of this.edges){const A=fm.get(e.aId),B=fm.get(e.bId);if(!A||!B){e.switch=.01;continue;}const pred=relativePose(A.poseEstimate||A.posePrior,B.poseEstimate||B.posePrior),r=relativeResidual(e.observed,pred),te=Math.hypot(r[0],r[1],r[2]),re=Math.hypot(r[3],r[4],r[5]),rgb=Math.sqrt(frameMapValue(rgbFrameSupport,e.aId,.6)*frameMapValue(rgbFrameSupport,e.bId,.6)),imp=.5*(frameMapValue(poseImprovement,e.aId,0)+frameMapValue(poseImprovement,e.bId,0));
      const tLike=Math.exp(-.5*(te/this.translationSigma)**2),rLike=Math.exp(-.5*(re/this.rotationSigmaRad)**2),contradiction=clamp(imp*rgb,0,.95),tPost=bayesSwitch(e.prior,tLike*(1-.78*contradiction)),rPost=bayesSwitch(e.prior,rLike*(1-.42*contradiction));e.translationSwitch=clamp(.25*e.translationSwitch+.75*tPost,.003,.998);e.rotationSwitch=clamp(.25*e.rotationSwitch+.75*rPost,.003,.998);e.switch=Math.sqrt(e.translationSwitch*e.rotationSwitch);e.translationResidual=te;e.rotationResidualRad=re;e.rgbSupport=rgb;e.poseImprovement=imp;}return this.stats();}
