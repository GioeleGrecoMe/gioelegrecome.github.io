...
export class ProbabilisticFactorGraph{
  ...
  addAlvaRelativeFactor(a,b){
    if(!a?.posePrior||!b?.posePrior)return null;const aId=String(a.frameId),bId=String(b.frameId);if(!aId||!bId||aId===bId)return null;const dp=[b.posePrior.p[0]-a.posePrior.p[0],b.posePrior.p[1]-a.posePrior.p[1],b.posePrior.p[2]-a.posePrior.p[2]],qinv=qConjLocal(a.posePrior.q),p=qRotateLocal(qinv,dp),q=qMulLocal(qinv,b.posePrior.q),item={aId,bId,relativePose:{p,q:qNormalizeLocal(q)},priorConfidence:alvaPriorConfidence(a,b),translationSwitch:1,rotationSwitch:1};this.alvaFactors.push(item);trim(this.alvaFactors,this.maxFrames*2);return item;
  }
