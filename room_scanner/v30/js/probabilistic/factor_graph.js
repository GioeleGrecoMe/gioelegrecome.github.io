...
export class ProbabilisticFactorGraph{
  constructor({maxFrames=360,maxFeaturesPerFrame=360,grayMaxSide=120,photoMaxSide=128,deepGridCols=32,deepGridRows=48,mvsPerFrame=420,maxLandmarks=18000}={}){
    ...
    this.frames=[];this.frameIndex=new Map();this.landmarkFactors=[];this.deepFactors=[];this.mvsFactors=[];this.edgeFactors=[];this.alvaFactors=[];this.measurementIndex=new Map();this.cameraModel=null;this.version=5;this.createdAt=Date.now();
  }
  addFrame(frame){
  ...
  addPhotoEdges(edges){
    const live=new Set(this.frames.map(f=>String(f.frameId))),out=[];for(const e of edges||[]){const aId=resolvePhotoFrameId(e,'a',this.frames),bId=resolvePhotoFrameId(e,'b',this.frames);if(!aId||!bId||aId===bId||!live.has(aId)||!live.has(bId))continue;out.push({aId,bId,loop:!!e.loop,visualConfidence:clamp(Number(e.visualConfidence??e.weight??.1),.001,.999),rotationBToA:Array.isArray(e.rotationBToA)&&e.rotationBToA.length===9?e.rotationBToA.slice(0,9).map(Number):null,matches:(e.matches||[]).slice(0,180).map(m=>({aU:+m.aU,aV:+m.aV,bU:+m.bU,bV:+m.bV,probability:clamp(Number(m.probability??.1),.001,.999),photometricProbability:m.photometricProbability==null?null:+m.photometricProbability}))});}this.edgeFactors=out;return out.length;
  }
  ...
  addMvs(frameId,samples,{sourceFrames=[]}={}){
    ...
    this.mvsFactors.push({frameId:fid,sourceFrames:src,count:n,data,normal,normalSpace:qinv?'camera':'unknown',color,flags,viewMask,packed:true});trim(this.mvsFactors,this.maxFrames);
  }
  exportState(){return {format:'ROOMSCAN-PROB-GRAPH-1',version:this.version,createdAt:this.createdAt,cameraModel:this.cameraModel,frames:this.frames,edgeFactors:this.edgeFactors,alvaFactors:this.alvaFactors,landmarkFactors:this.landmarkFactors,deepFactors:this.deepFactors,mvsFactors:this.mvsFactors,summary:this.summary()};}
  summary(){const obs=this.landmarkFactors.reduce((n,x)=>n+x.measurements.length,0),mvs=this.mvsFactors.reduce((n,x)=>n+(x.count??x.samples?.length??0),0);return {frames:this.frames.length,photoEdges:this.edgeFactors.length,alvaEdges:this.alvaFactors.length,landmarks:this.landmarkFactors.length,featureObservations:obs,deepFrames:this.deepFactors.length,mvsSamples:mvs,cameraFixed:!!this.cameraModel,bytesApprox:this.approxBytes()};}
