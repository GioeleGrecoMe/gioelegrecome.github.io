...
export class ProbabilisticFactorGraph{
  constructor({maxFrames=360,maxFeaturesPerFrame=360,grayMaxSide=120,photoMaxSide=128,deepGridCols=32,deepGridRows=48,mvsPerFrame=420,maxLandmarks=18000}={}){
    ...
    this.frames=[];this.frameIndex=new Map();this.landmarkFactors=[];this.deepFactors=[];this.mvsFactors=[];this.edgeFactors=[];this.alvaFactors=[];this.measurementIndex=new Map();this.cameraModel=null;this.photoEdgeAudit={inputEdges:0,importedEdges:0,unresolvedEdges:0,importFraction:1};this.version=6;this.createdAt=Date.now();
  }
  addFrame(frame){
  ...
  addPhotoEdges(edges){
    const input=Array.isArray(edges)?edges:[],live=new Set(this.frames.map(f=>String(f.frameId))),out=[];let unresolved=0;
    for(const e of input){const aId=resolvePhotoFrameId(e,'a',this.frames),bId=resolvePhotoFrameId(e,'b',this.frames);if(!aId||!bId||aId===bId||!live.has(aId)||!live.has(bId)){unresolved++;continue;}out.push({aId,bId,loop:!!e.loop,visualConfidence:clamp(Number(e.visualConfidence??e.weight??.1),.001,.999),rotationBToA:Array.isArray(e.rotationBToA)&&e.rotationBToA.length===9?e.rotationBToA.slice(0,9).map(Number):null,matches:(e.matches||[]).slice(0,180).map(m=>({aU:+m.aU,aV:+m.aV,bU:+m.bU,bV:+m.bV,probability:clamp(Number(m.probability??.1),.001,.999),photometricProbability:m.photometricProbability==null?null:+m.photometricProbability}))});}
    this.edgeFactors=out;this.photoEdgeAudit={inputEdges:input.length,importedEdges:out.length,unresolvedEdges:unresolved,importFraction:input.length?out.length/input.length:1};return out.length;
  }
  ...
  addMvs(frameId,samples,{sourceFrames=[]}={}){
    ...
    const fid=String(frameId),frame=this.frames[this.frameIndex.get(fid)],qinv=frame?.posePrior?.q?qConjLocal(frame.posePrior.q):null,step=Math.max(1,Math.ceil(valid.length/this.mvsPerFrame)),picked=[];for(let i=0;i<valid.length;i+=step)picked.push(valid[i]);const n=picked.length,data=new Float32Array(n*8),normal=new Float32Array(n*3),color=new Uint8Array(n*3),flags=new Uint8Array(n),viewMask=new Uint16Array(n);
    ...
    this.mvsFactors.push({frameId:fid,sourceFrames:[...new Set(src.filter(x=>x&&x!==fid))],count:n,data,normal,normalSpace:qinv?'camera':'unknown',color,flags,viewMask,packed:true,depthFrame:'reference-camera-z',estimatedUnder:'posePrior',finalPoseRevalidationRequired:true});trim(this.mvsFactors,this.maxFrames);
  }
  ...
  summary(){const obs=this.landmarkFactors.reduce((n,x)=>n+x.measurements.length,0),mvs=this.mvsFactors.reduce((n,x)=>n+(x.count??x.samples?.length??0),0),audit=this.photoEdgeAudit||{inputEdges:this.edgeFactors.length,importedEdges:this.edgeFactors.length,unresolvedEdges:0,importFraction:1};return {frames:this.frames.length,photoEdges:this.edgeFactors.length,photoEdgeInput:audit.inputEdges,photoEdgeUnresolved:audit.unresolvedEdges,photoEdgeImportFraction:audit.importFraction,alvaEdges:this.alvaFactors.length,landmarks:this.landmarkFactors.length,featureObservations:obs,deepFrames:this.deepFactors.length,mvsSamples:mvs,cameraFixed:!!this.cameraModel,bytesApprox:this.approxBytes()};}
