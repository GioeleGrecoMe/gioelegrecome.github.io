...
export class SwitchablePhotoEdgeModel{
  ...
  update(frames,landmarks,{bootstrap=false}={}){const fm=new Map((frames||[]).map((f,i)=>[String(f.frameId),{f,i}]));for(const e of this.edges){const aId=String(e.aId??e.a),bId=String(e.bId??e.b),A=fm.get(aId)?.f,B=fm.get(bId)?.f;if(!A||!B){e.switch=Math.min(e.switch,.08);continue;}const rotErr=rotationResidualRad(A.poseEstimate||A.posePrior,B.poseEstimate||B.posePrior,e.rotationBToA),shared=sharedLandmarkResidual(aId,bId,A,B,landmarks),prior=clamp(Number(e.visualConfidence??e.weight??.15),this.minPrior,.995),matchSupport=clamp(Math.log1p((e.matches?.length||e.inliers||0))/Math.log(30),.08,1),sharedStrength=clamp(shared.count/14,0,1),rotLike=Math.exp(-.5*(rotErr/this.rotationSigmaRad)**2),landLike=shared.count?Math.exp(-.5*(shared.median/this.reprojectionSigmaPx)**2):.62;
      // Missing matches/shared landmarks are absence of evidence, not evidence of
      // a bad edge. The old multiplicative "likelihood" treated low support as a
      // negative observation and then used 1-likelihood as P(data|bad), causing
      // nearly every RGB edge to collapse toward zero. Use bounded log-odds
      // evidence instead: geometry decides direction, support decides strength.
      const landWeight=.22+.58*sharedStrength,structural=clamp((1-landWeight)*rotLike+landWeight*landLike,0,1),strength=clamp(.10+.38*matchSupport+.52*sharedStrength,.10,1),logitPrior=Math.log(prior/Math.max(1e-9,1-prior)),evidence=clamp((structural-.48)*6,-4,4),post=1/(1+Math.exp(-(logitPrior+strength*evidence))),alpha=bootstrap?.18:.58,bootstrapFloor=bootstrap?clamp(prior*.72,.08,.32):clamp(prior*.05,.004,.05);e.switch=clamp((1-alpha)*e.switch+alpha*post,bootstrapFloor,.998);e.rotationResidualRad=rotErr;e.sharedLandmarks=shared.count;e.sharedReprojectionPx=shared.median;e.posterior=post;e.structuralLikelihood=structural;e.evidenceStrength=strength;e.bootstrapFloor=bootstrap?bootstrapFloor:null;}return this.stats();}
