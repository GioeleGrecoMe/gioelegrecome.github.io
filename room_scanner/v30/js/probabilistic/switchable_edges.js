...
export class SwitchablePhotoEdgeModel{
  ...
  update(frames,landmarks,{bootstrap=false}={}){const fm=new Map((frames||[]).map((f,i)=>[String(f.frameId),{f,i}]));for(const e of this.edges){const aId=String(e.aId??e.a),bId=String(e.bId??e.b),A=fm.get(aId)?.f,B=fm.get(bId)?.f;if(!A||!B){e.switch=Math.min(e.switch,.08);continue;}const rotErr=rotationResidualRad(A.poseEstimate||A.posePrior,B.poseEstimate||B.posePrior,e.rotationBToA),shared=sharedLandmarkResidual(aId,bId,A,B,landmarks),prior=clamp(Number(e.visualConfidence??e.weight??.15),this.minPrior,.995),matchSupport=clamp(Math.log1p((e.matches?.length||e.inliers||0))/Math.log(30),.10,1),rotLike=Math.exp(-.5*(rotErr/this.rotationSigmaRad)**2),landLike=shared.count?Math.exp(-.5*(shared.median/this.reprojectionSigmaPx)**2):.58;const likelihood=clamp(rotLike*(.22+.78*landLike)*(.28+.72*matchSupport),.0005,.9995),po=prior/(1-prior),odds=po*likelihood/Math.max(.02,1-likelihood),post=odds/(1+odds),alpha=bootstrap?.18:.85,bootstrapFloor=bootstrap?clamp(prior*.72,.08,.32):.002;e.switch=clamp((1-alpha)*e.switch+alpha*post,bootstrapFloor,.998);e.rotationResidualRad=rotErr;e.sharedLandmarks=shared.count;e.sharedReprojectionPx=shared.median;e.posterior=post;e.bootstrapFloor=bootstrap?bootstrapFloor:null;}return this.stats();}
