/**
 * Room Scanner V30.42.0 final-pose dense consensus application.
 *
 * BOOT CONTRACT
 * -------------
 ...
 */
import {BUILD,CONFIG} from './config.js?v=30.42.0';
import {DiagnosticsLog} from './logger.js?v=30.42.0';
...
async function beginBridge(){
  // V30.42.0: only ProbabilisticJointOptimizer is operational; committed surface is rebuilt globally from confirmed dense evidence.
...
async function startProbabilisticOptimization(){
  ...
  try{
    ...
    if(rebuilt?.withheldReason){state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;window.__ROOMSCAN_METRIC_MESH=null;state.surfaceStats={...(state.surfaceStats||{}),committed:false,committedGaussians:0,committedFaces:0,withheldReason:rebuilt.withheldReason};state.renderer?.setData([],{fit:false});state.renderer?.setMesh(null);state.renderer?.draw();log.warn('committed-surface-withheld',{reason:rebuilt.withheldReason,rgbEdges:rebuilt.stats?.edgeSwitches||null,alvaEdges:rebuilt.stats?.alvaSwitches||null,reprojectionRobustRmse:rebuilt.stats?.reprojectionRobustRmse??null,reprojectionOptimizationRobustRmse:rebuilt.stats?.reprojectionOptimizationRobustRmse??null,advice:'RGB consensus is not independently strong enough to authorize dense geometry'});}
    if(rebuilt?.map){const committed=rebuilt.map.gaussians||[],mq=rebuilt.map.stats?.meshQuality||null,mvsV=rebuilt.map.stats?.mvsValidation||null;state.probOptimized={previewGaussians:committed,candidateGaussians:rebuilt.map.candidateGaussians||[],mapStats:rebuilt.map.stats};if(committed.length){state.gaussians=committed;state.mesh=rebuilt.map.mesh?.vertices?.length?rebuilt.map.mesh:null;window.__ROOMSCAN_METRIC_MESH=state.mesh;state.meshStale=!state.mesh;state.geometryCommitted=true;state.surfaceStats={...(state.surfaceStats||{}),committed:true,committedGaussians:state.gaussians.length,committedFaces:state.mesh?.faces?.length?state.mesh.faces.length/3:0,meshQuality:mq,surfaceLayers:rebuilt.map.mesh?.surfaceLayers??null,meshedSurfelFraction:rebuilt.map.mesh?.meshedSurfelFraction??null,eligibleCommittedFrames:rebuilt.map.stats?.eligibleCommittedFrames??null,excludedUnacceptedFrames:rebuilt.map.stats?.excludedUnacceptedFrames??null,mvsValidation:mvsV};state.renderer?.setData(state.gaussians,{fit:false});state.renderer?.setMesh(state.mesh);state.renderer?.draw();if(mq?.status==='fragmented')log.warn('committed-mesh-fragmented',{meshQuality:mq,surfaceLayers:rebuilt.map.mesh?.surfaceLayers??null,meshedSurfelFraction:rebuilt.map.mesh?.meshedSurfelFraction??null,gaussians:state.gaussians.length,faces:state.mesh?.faces?.length?state.mesh.faces.length/3:0,mvsValidation:mvsV,advice:'mesh kept for diagnostics; inspect final-pose MVS validation and RGB consensus before trusting export'});if(mvsV&&mvsV.commitFraction<.25)log.warn('committed-mvs-support-low',{mvsValidation:mvsV,advice:'most stored MVS proposals failed final-pose photometric revalidation; surface is intentionally sparse rather than fabricated'});}else{state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;state.surfaceStats={...(state.surfaceStats||{}),committed:false,committedGaussians:0,committedFaces:0,meshQuality:mq,mvsValidation:mvsV};state.renderer?.setData([],{fit:false});state.renderer?.setMesh(null);state.renderer?.draw();log.warn('committed-surface-withheld',{reason:'no-dense-evidence-survived-final-pose-validation',mvsValidation:mvsV,depthCalibration:rebuilt.map.stats?.depthCalibration||null,rgbEdges:rebuilt.map.stats?.edgeSwitches||null});}}
