...
function updateOptimizerUi(extra=null){
  ...
  if(status)status.textContent=extra||`OPT UNICO · acc ${current} · factor graph ${graph?`${graph.frames}F / ${graph.landmarks}L / ${graph.photoEdges} RGB edge / ${graph.alvaEdges} Alva / ${graph.deepFrames} Deep`:'non disponibile'}${state.liveOptStats?.reprojectionRmse!=null?` · reproj ${Number(state.liveOptStats.reprojectionRmse).toFixed(2)} px`:''}${state.liveOptGate?.hardReasons?.length?` · ultimo gate: ${state.liveOptGate.hardReasons.join(', ')}`:''}`;
}
...
async function startProbabilisticOptimization(){
  ...
  const runtime=await ensureSingleOptimizerRuntime();runtime.reset(state.probOptimization||state.liveOptAccepted||null);runtime.resume();
  ...
  try{
    while(state.postOptBusy&&accepted<target&&attempts<maxAttempts){attempts++;const result=await runtime.runCycle({mode:'review',reason:'review-manual',graph,budgetMs:250,maxIterations:1,previewMap:false,options:{maxLandmarks:CONFIG.probabilisticMaxLandmarks||12000,maxObsPerFrame:CONFIG.probabilisticMaxObsPerFrame||280,posePriorScale:CONFIG.probabilisticPosePriorScale||1,absoluteAlvaScale:CONFIG.probabilisticAbsoluteAlvaScale||.04,depthFeedbackEvery:CONFIG.probabilisticDepthFeedbackEvery||2,localWindowSize:CONFIG.liveOptSlowGraphFrames||26,localWindowOverlap:CONFIG.liveOptGraphLoopFrames||6},gateOptions:{maxReprojectionPx:CONFIG.liveOptGateMaxReprojectionPx||3.2,maxCommonTranslationJump:state.reviewMetricLocked?(CONFIG.liveOptGateMaxTranslationM||.11):(CONFIG.liveOptGateMaxTranslationAlva||.14),maxCommonRotationJumpRad:CONFIG.liveOptGateMaxRotationRad||.07,maxMeanTranslationJump:state.reviewMetricLocked?(CONFIG.liveOptGateMeanTranslationM||.045):(CONFIG.liveOptGateMeanTranslationAlva||.06)}});last=result;
      ...
      if(result.type==='single-opt-accepted'){state.liveOptAccepted=result.snapshot;state.liveOptStats=result.stats;state.liveOptGate=result.gate;state.liveOptAcceptedCount++;accepted=Number(result.snapshot?.iterations||result.stats?.iterations||accepted+1);state.probOptimization={...(result.snapshot||{}),iterations:accepted,stats:result.stats,updatedAt:Date.now()};state.optimization={...(state.optimization||{}),iterations:accepted,lastEnergy:result.stats?.energy??state.optimization?.lastEnergy,updatedAt:Date.now()};log.decision('single-opt-review-accepted',{attempt:attempts,accepted,target,gate:result.gate,stats:result.stats});}
      else if(result.type==='single-opt-rejected'){state.liveOptRejected++;log.warn('single-opt-review-rejected',{attempt:attempts,accepted,target,gate:result.gate,stats:result.stats});}
      updateReviewUi();updateOptimizerUi(`OPT UNICO · acc ${accepted}/${target} · tentativi ${attempts} · reproj ${Number(state.liveOptStats?.reprojectionRmse||0).toFixed(2)} px · fase ${state.liveOptStats?.feedbackPhase||'—'} · rifiutati ${state.liveOptRejected}`);await new Promise(r=>setTimeout(r,0));
    }
    if(!state.postOptBusy){updateOptimizerUi('OPT UNICO fermato · stato accettato conservato.');return;}
    const rebuilt=await runtime.rebuildAccepted(graph,{optimizer:{maxLandmarks:CONFIG.probabilisticMaxLandmarks||12000,maxObsPerFrame:CONFIG.probabilisticMaxObsPerFrame||280,posePriorScale:CONFIG.probabilisticPosePriorScale||1,absoluteAlvaScale:CONFIG.probabilisticAbsoluteAlvaScale||.04,depthFeedbackEvery:CONFIG.probabilisticDepthFeedbackEvery||2},rebuild:rebuildOptions});
    ...
    log.info('single-opt-review-complete',{accepted,attempts,target,stats:state.liveOptStats,map:rebuilt?.map?.stats||null,graph:state.probGraph.summary?.()||null});state.postOptBusy=false;await persistCurrentSession({status:'optimized',keyframes:state.reviewKeyframes||0}).catch(err=>log.warn('single-opt-persist',{message:err?.message||String(err)}));await renderSessions().catch(()=>{});updateReviewUi();updateOptimizerUi(`OPT UNICO completato · ${accepted} iterazioni accettate / ${attempts} tentativi · committed ${state.gaussians.length}${state.mesh?` · ${state.mesh.faces.length/3} facce`:''}.`);
