/**
 * Room Scanner V30.29.0 spherical photo-puzzle + hybrid plane/particle reconstruction application.
 *
 * BOOT CONTRACT
 * -------------
...
async function startScan(metric={},epoch=state.bridgeEpoch,bridge=null){
  if(epoch!==state.bridgeEpoch)return;
  bridge?.pause?.();
  const [{CameraController,intrinsicsForCrop},{SlamEngine},{LiveReconstructionOverlay},{DenseKeyframeManager},{DeepKeyframeSelector},deepSync,coverageApi]=await Promise.all([
    lazy('./camera.js'),lazy('./slam/slam_engine.js'),lazy('./gaussian/ar_overlay.js'),lazy('./dense/keyframe_manager.js'),lazy('./dense/deep_keyframe_selector.js'),lazy('./dense/deep_frame_sync.js'),lazy('./reconstruction/coverage_sphere.js')
  ]);
  state.deepSync=deepSync;state.coverageApi=coverageApi;
  if(epoch!==state.bridgeEpoch)return;
