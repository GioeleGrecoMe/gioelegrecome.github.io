/**
 * Room Scanner V30.49.0 sharp RGB archive + visual post-scan Deep processing.
 *
 * BOOT CONTRACT
 * -------------
...
async function startScan(metric={},epoch=state.bridgeEpoch,bridge=null){
  ...
  state.scanStop=state.camera.loop(frame=>{try{
    ...
    // FAST LANE: freeze every sufficiently still Alva-valid RGB frame into the
    // post-scan bank. No photo matching, Deep or plane sweep runs here.
...
function registerDepthPlannedPhoto(survey,{source='depth-planned',optimize=true,render=true}={}){
  ...
  if(graphFrame&&optimize)scheduleLiveOptimization('rgb-depth-planned',{slow:false});
...
function compactArchiveFeatures(xs,max=180){return (xs||[]).slice(0,max).map(f=>({x:+f.x||0,y:+f.y||0,score:+f.score||0,source:f.source||'rgb',desc:Array.isArray(f.desc)?f.desc.slice(0,24).map(Number):[]}));}
function archiveSharpRgbFrame(frame,tracking){
  if(CONFIG.sharpPhotoArchiveEnabled===false||!state.currentSession||!frame?.rgba?.length||!frame?.gray?.length||!tracking?.trackingValid||!tracking?.pose?.p||!tracking?.pose?.q)return false;
...
function selectSharpPhotosForProcessing(rows,maxFrames=300){
  if(rows.length<=maxFrames)return rows.slice();const must=new Map();must.set(0,rows[0]);must.set(rows.length-1,rows[rows.length-1]);rows.forEach((r,i)=>{if(r.stability?.jumpSuspect)must.set(i,r);});const remaining=Math.max(1,maxFrames-must.size),picked=new Map(must),bin=rows.length/remaining;
  for(let b=0;b<remaining;b++){const a=Math.floor(b*bin),z=Math.min(rows.length,Math.max(a+1,Math.floor((b+1)*bin)));let best=-1,bq=-Infinity;for(let i=a;i<z;i++){if(picked.has(i))continue;const r=rows[i],q=Number(r.photoQuality?.stableQuality||0)+.035*Math.log1p(Number(r.photoQuality?.detail)||0);if(q>bq){bq=q;best=i;}}if(best>=0)picked.set(best,rows[best]);}
  return [...picked.entries()].sort((a,b)=>a[0]-b[0]).slice(0,maxFrames).map(x=>x[1]);
}
async function decodeSharpPhotoRecord(record){
  if(record?.rgba?.length&&record?.gray?.length)return {...record,rgba:new Uint8ClampedArray(record.rgba),gray:new Uint8Array(record.gray)};if(!record?.blob)throw new Error('archived photo has no raster');const bitmap=await createImageBitmap(record.blob),w=bitmap.width|0,h=bitmap.height|0,canvas=typeof OffscreenCanvas!=='undefined'?new OffscreenCanvas(w,h):document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d',{willReadFrequently:true});ctx.drawImage(bitmap,0,0);bitmap.close?.();const image=ctx.getImageData(0,0,w,h),rgba=new Uint8ClampedArray(image.data),gray=new Uint8Array(w*h);for(let i=0;i<gray.length;i++)gray[i]=Math.max(0,Math.min(255,Math.round(.299*rgba[4*i]+.587*rgba[4*i+1]+.114*rgba[4*i+2])));return {...record,width:w,height:h,rgba,gray,K:{...(record.K||state.scanK)}};
}
...
function processingSetPhase(phase,{index=0,total=state.processingPhotos?.length||0,message='',detail=''}={}){
  ...
  const label=phase==='rgb'?'REGISTRAZIONE RGB':phase==='scaffold'?'SCAFFOLD RGB + ALVA':phase==='deep'?'DEEP PRIOR':phase==='mvs'?'MVS POST-SCAN':phase==='final'?'FUSIONE FINALE':String(phase||'PROCESSING').toUpperCase();
...
function showProcessingFrame(photo,index,total,{phase='rgb',deep=null,message=''}={}){
  if(!photo)return;drawRasterToCanvas($('processingRgb'),photo.rgba,photo.width,photo.height);if(deep)drawProcessingDepth(deep);else drawProcessingDepth(null);drawProcessingPose(photo,index,total,deep);
...
async function registerArchivedPhotosPostScan(records){
  state.processingRgbImported=0;state.processingPoseJumps=0;state.processingPoseList=records.map(r=>({frameId:r.frameId,at:r.at,pose:r.pose,stability:r.stability,photoQuality:r.photoQuality}));processingSetPhase('rgb',{index:0,total:records.length,message:`Registro ${records.length} fotografie nitide nel grafo RGB`});
  const yieldEvery=Math.max(1,Number(CONFIG.sharpArchiveProcessingYieldEvery)||3);for(let i=0;i<records.length;i++){const decoded=await decodeSharpPhotoRecord(records[i]),survey=archiveRecordToSurvey(decoded);if(survey.stability?.jumpSuspect)state.processingPoseJumps++;showProcessingFrame(survey,i,records.length,{phase:'rgb'});const reg=registerDepthPlannedPhoto(survey,{source:'sharp-rgb-postscan',optimize:false,render:false});if(reg?.ok)state.processingRgbImported++;if((i+1)%yieldEvery===0)await new Promise(r=>setTimeout(r,0));}
...
async function processArchivedDeepSequential(records){
  if(CONFIG.deepDepthEnabled===false||state.deepDisabled||!records.length)return {processed:0,failed:0};try{ensureDeepRuntimeWorker();}catch(err){state.deepDisabled=true;log.warn('processing-deep-init',{message:err?.message||String(err)});return {processed:0,failed:records.length};}
  state.deepLateQueue?.reset?.();state.deepLaneQueuedFrames?.clear?.();state.deepJobs.clear();state.deepLaneInFlight=null;state.deepDrainActive=true;let processed=0,failed=0;const timeout=Math.max(15000,Number(CONFIG.sharpArchiveDeepFrameTimeoutMs)||120000);log.info('processing-deep-start',{frames:records.length,timeoutPerFrameMs:timeout});
  for(let i=0;i<records.length;i++){if(state.processingAbort)break;let decoded,survey;try{decoded=await decodeSharpPhotoRecord(records[i]);survey=archiveRecordToSurvey(decoded);showProcessingFrame(survey,i,records.length,{phase:'deep',message:`Avvio DeepPrior ${i+1}/${records.length} · ${survey.frameId}`});const jobId=`deep-archive-${survey.frameId}-${i}`,queued=enqueueLateDeepJob({jobId,kind:'preview',frameId:survey.frameId,frameAt:survey.at,refId:survey.id,rgba:new Uint8ClampedArray(survey.rgba),width:survey.width,height:survey.height,survey,tracking:{pose:survey.pose,featureCount:survey.features?.length||0,anchorCount:0}});if(!queued.ok){failed++;continue;}await pumpDeepLateLane();const ok=await waitForDeepJobCompletion(jobId,timeout);if(!ok){failed++;log.warn('processing-deep-timeout',{jobId,frameId:survey.frameId,timeoutMs:timeout});if(String(state.deepLaneInFlight||'')===jobId)state.deepLaneInFlight=null;state.deepJobs.delete(jobId);state.deepLateQueue?.fail?.(jobId);}else processed++;}catch(err){failed++;log.warn('processing-deep-frame',{index:i,frameId:records[i]?.frameId||null,message:err?.message||String(err)});}if((i+1)%2===0)await new Promise(r=>setTimeout(r,0));}
  state.deepDrainActive=false;log.info('processing-deep-complete',{requested:records.length,processed,failed,accepted:state.processingDeepAccepted});return {processed,failed,accepted:state.processingDeepAccepted};
}
...
async function finishScan(){
  ...
  // Every selected RGB frame is explicitly depth-planned and receives Deep on
  // the exact frozen raster. Queueing is sequential so hundreds of photos do
  // not coexist as uncompressed RGBA buffers on a phone.
  const preplannedDeepDrain=await drainDeepBacklog();
  const archivedDeepDrain=selected.length?await processArchivedDeepSequential(selected):{processed:0,failed:0,accepted:0};
  const deepDrain={preplanned:preplannedDeepDrain,archive:archivedDeepDrain};
  processingSetPhase('mvs',{index:0,total:Math.max(1,state.postScanMvsPayloads?.length||0),message:'MVS sulle pose finali…'});
  const mvsDrain=await drainPostScanMvsBacklog(rgbScaffold?.snapshot||null);processingSetPhase('final',{index:1,total:1,message:'Fusione e salvataggio finale…'});
  ...
  state.processingActive=false;processingSetPhase('final',{index:1,total:1,message:'Processing completato · apro REVIEW'});state.processingActive=false;
