/**
 * Room Scanner V30.46.0 canonical RGB scaffold + post-scan MVS/Deep application.
 *
 * BOOT CONTRACT
 * -------------
...
async function beginBridge(){
  // V30.46.0: only ProbabilisticJointOptimizer is operational; Deep is late-bound and cannot stall the RGB/Alva fast lane.
...
async function startScan(metric={},epoch=state.bridgeEpoch,bridge=null){
  ...
  if($('mvsState'))$('mvsState').textContent='FAST: Alva+RGB+MVS · Deep accodata per post-scan';
  if($('deepLiveState'))$('deepLiveState').textContent='DEEP POST · coda vuota';
  ...
  state.scanStop=state.camera.loop(frame=>{try{
    state.lastFrameGeometry=frame.geometry;noteFastLaneFrame(frame);const r=state.slam.process(frame);state.lastTracking=r;
    ...
    // FAST LANE: capture only an immutable RGB frame and enqueue its future Depth.
    // No neural inference is allowed to block Alva/RGB while scanning.
...
function captureLiveSurveyFrame(frame,tracking){
  // Freeze the exact RGB frame that is explicitly depth-planned. V30.46 may
...
function registerDepthPlannedPhoto(survey,{source='depth-planned'}={}){
  if(!survey?.frameId)return {ok:false,reason:'missing-frame'};
  const id=String(survey.frameId);if(state.photoPlannedFrameIds?.has?.(id))return {ok:true,deduplicated:true,method:'already-planned'};
  ...
  if(state.liveMap){
    ...
    for(const name of candidates){
      ...
      try{
        const before=state.liveMap?.stats?.()?.frames||0,r=fn.call(state.liveMap,survey,{depthPlanned:true,source,fallbackDepth:state.denseDepthHint||CONFIG.livePuzzleFallbackDepth||2.2}),after=state.liveMap?.stats?.()?.frames||0,exported=state.liveMap?.exportState?.(),present=!!exported?.frames?.some?.(f=>String(f?.frameId||'')===id);
  ...
  state.photoPlannedFrameIds?.add?.(id);
  if(mapOk){state.liveMapStats=state.liveMap?.stats?.()||state.liveMapStats;scheduleLiveMapRender(true);}
  log.info('rgb-depth-planned-photo',{frameId:id,source,mapRegistered:mapOk,mapMethod:method,metricGraphNode:!!graphFrame,error,queue:state.deepLateQueue?.stats?.()||null});
  if(graphFrame)scheduleLiveOptimization('rgb-depth-planned',{slow:false});
...
function enqueueLateDeepJob(job){
  ...
  state.deepLaneQueuedFrames?.add?.(String(job.frameId));
...
function requestLiveDeepPreview(frame,tracking){
  if(!frame?.rgba?.length||!frame?.gray?.length||!state.scanK||CONFIG.deepDepthEnabled===false||state.deepDisabled||!state.deepSync)return;
  const now=Number(frame.at)||performance.now(),interval=Math.max(700,Number(CONFIG.deepPlanIntervalMs||CONFIG.deepInferenceIntervalMs)||1800);
  if(now-state.deepPreviewLastAt<interval)return;
  const qstats=state.deepLateQueue?.stats?.()||{};if((qstats.surveys||0)>=(CONFIG.deepSurveyQueueBudget||8))return;
  const capture=captureLiveSurveyFrame(frame,tracking);if(!capture?.survey)return;
  const survey=capture.survey,jobId=`preview-ticker-${++state.deepPreviewSeq}-${Math.round(now)}`,rgba=new Uint8ClampedArray(survey.rgba);
  const queued=enqueueLateDeepJob({jobId,kind:'preview',frameId:survey.frameId,frameAt:survey.at,rgba,width:survey.width,height:survey.height,survey,tracking:{frameId:tracking?.frameId||null,trackingValid:!!tracking?.trackingValid,trackingMode:tracking?.trackingMode||null,pose:tracking?.pose?{p:[...tracking.pose.p],q:[...tracking.pose.q]}:null,poseCov:tracking?.poseCov||null,alvaPoints:tracking?.alvaPoints||0,keyframeId:tracking?.newKeyframe?.id||null}});
  if(!queued.ok)return;
  // The RGB frame is admitted NOW because Depth is explicitly planned for this
  // exact immutable frame. Deep may arrive seconds later or after Scan ends.
  registerDepthPlannedPhoto(survey,{source:'survey-depth-planned'});
  state.photoSurveyLastAt=now;state.deepPreviewLastAt=now;
  if($('deepLiveState'))$('deepLiveState').textContent=`RGB FAST ✓ · Deep in coda ${state.deepLateQueue?.stats?.().queued||0}`;
}
...
async function scheduleSparseGeometryWork(){
  ...
  try{
    ...
    payload.jobId=`sparse-${++state.sparseJobs}`;planLateDeepFromPayload(payload);stripDeepRaster(payload);const retained=retainPostScanMvsPayload(payload);
...
async function drainPostScanMvsBacklog(snapshot=null){
  ...
  for(const payload of queue){if(!payload?.ref)continue;await waitForDenseIdle(20000);payload.jobId=`mvs-post-${++state.denseJobs}`;await dispatchDensePayload(payload);const deadline=performance.now()+Math.max(3000,Number(CONFIG.postScanMvsJobTimeoutMs)||18000);while(state.denseBusy&&performance.now()<deadline)await new Promise(r=>setTimeout(r,30));if(state.denseBusy){timeouts++;state.denseBusy=false;state.denseActivePayload=null;log.warn('mvs-postscan-timeout',{jobId:payload.jobId,frameId:payload.ref.frameId});}else processed++;await new Promise(r=>setTimeout(r,0));}
  state.postScanMvsDrainActive=false;state.postScanMvsPayloads=[];const out={processed,timeouts,elapsedMs:performance.now()-started,mvsSamples:state.denseDepthSamples};log.info('mvs-postscan-drain-complete',out);return out;
}
async function reconcilePostScanRgbScaffold(){
  if(!state.probGraph)return null;const panorama=state.liveMap?.exportState?.()||null;if(panorama?.edges?.length)state.probGraph.addPhotoEdges?.(panorama.edges);const runtime=await ensureSingleOptimizerRuntime(),graph=state.probGraph.exportState(),result=await runtime.reconcileRgbScaffold?.(graph,{passes:CONFIG.postScanRgbScaffoldPasses||20,optimizer:{localWindowSize:CONFIG.postOptimizeLocalWindowFrames||20,localWindowOverlap:CONFIG.postOptimizeLocalWindowOverlap||6}});state.postScanRgbScaffold=result?{accepted:!!result.accepted,policy:result.candidatePolicy||result.baselinePolicy||null,stats:result.stats||null}:null;log.checkpoint('postscan-rgb-scaffold',{accepted:!!result?.accepted,policy:result?.candidatePolicy||result?.baselinePolicy||null,stats:result?.stats||null,photoAudit:state.probGraph.photoEdgeAudit||null});return result;
}
...
async function handleDeepDepthMessage(d){
  ...
  try{
    ...
    if(binding.kind==='preview'){
      ...
      const survey=binding.survey,q=d.quality||{},attached=survey?attachLateDepthToPhoto(survey,d,{source:'deep-postscan-survey'}):null;
...
async function applyDeepDepthResult(d,payload,{lateBound=true}={}){
  ...
  const denseSurvey=denseKeyframeSurvey(payload.ref),depthAttach=denseSurvey?attachLateDepthToPhoto(denseSurvey,d,{source:'deep-postscan-keyframe'}):null;
  ...
  // V30.46 deliberately stores RAW relative depth only. Any metric mapping made
...
async function finishScan(){
  ...
  // V30.46 post-scan ordering is deliberate: first solve the complete RGB/Alva
  // scaffold, then execute MVS on poses rebound to that solution, then attach
  // relative Deep. Neither dense source is allowed to own the acquisition clock.
  const rgbScaffold=await reconcilePostScanRgbScaffold();
  const mvsDrain=await drainPostScanMvsBacklog(rgbScaffold?.snapshot||null);
  await drainDeepBacklog();
  ...
  if(state.probGraph?.landmarkFactors?.length>=4){updateOptimizerUi('OPT UNICO · stato live salvato. Premi “Continua” per eseguire altri cicli globali espliciti.');}
}
...
async function persistCurrentSession({status='finished',keyframes=state.reviewKeyframes||0}={}){
  ...
  const snapshot={id,sessionId:id,format:'ROOMSCAN-PUZZLE-SESSION-5',savedAt:Date.now(),build:BUILD.id,gaussians:state.gaussians,optimizerObservations:state.optimizerObservations||null,factorGraph:graphState,deepSequence,photoPanorama,probOptimization:state.probOptimization||null,liveOptimization:state.liveOptAccepted?{snapshot:state.liveOptAccepted,stats:state.liveOptStats,gate:state.liveOptGate,accepted:state.liveOptAcceptedCount,rejected:state.liveOptRejected}:null,coverageSummary:coverageSnapshot(),optimization:{...(state.optimization||{iterations:0})},denseSamples:state.denseDepthSamples||0,deepRaySamples:state.deepRaySamples||0,metricLocked:state.reviewMetricLocked??!!state.slam?.metricLocked,geometryCommitted:!!state.geometryCommitted,keyframes};
...
async function showReview(){
  show('review');const {GaussianRenderer}=await lazy('./gaussian/renderer.js');if(!state.renderer)state.renderer=new GaussianRenderer($('viewer'));const candidateOnly=!state.gaussians.length&&!!state.denseCandidateGaussians?.length,displayGaussians=candidateOnly?state.denseCandidateGaussians:state.gaussians,displayMesh=state.mesh&&!state.meshStale?state.mesh:(candidateOnly?state.denseCandidateMesh:null);state.renderer.setData(displayGaussians,{fit:true});state.renderer.setMesh(displayMesh);state.renderer.draw();updateReviewUi();updateOptimizerUi();
}
function updateReviewUi(){
  const candidateOnly=!state.gaussians.length&&!!state.denseCandidateGaussians?.length,displayMesh=state.mesh&&!state.meshStale?state.mesh:(candidateOnly?state.denseCandidateMesh:null),faces=displayMesh?.faces?.length?displayMesh.faces.length/3:0,metric=state.reviewMetricLocked??!!state.slam?.metricLocked,kf=state.reviewKeyframes||state.slam?.keyframes?.length||0,iterations=state.optimization?.iterations||0,displaySplats=candidateOnly?state.denseCandidateGaussians.length:state.gaussians.length;
  if($('reviewStats'))$('reviewStats').textContent=`Surface splat: ${displaySplats}${candidateOnly?' candidati non committed':''} · depth sample ${state.denseDepthSamples||0} · scala ${metric?'metrica':'Alva libera'} · keyframe ${kf} · ottimizzazione ${iterations} iterazioni`;
  if($('metricGsStats'))$('metricGsStats').textContent=candidateOnly?`CANDIDATO NON COMMITTED: ${displayMesh?.vertices?.length?displayMesh.vertices.length/3:0} vertici / ${faces} facce · visualizzazione diagnostica soltanto, export disabilitato · motivo: ${state.surfaceStats?.withheldReason||'policy geometrica non soddisfatta'}.`:state.meshStale?'Mesh precedente non mostrata: le Gaussiane sono cambiate dopo l’ottimizzazione. La mesh per superfici verrà rigenerata in un passaggio dedicato.':state.mesh?`${state.geometryCommitted?'GEOMETRIA COMMITTED':'PREVIEW PROVVISORIA'}: ${state.mesh.vertices.length/3} vertici / ${faces} facce · ${state.geometryCommitted?'submap fuse dopo scaffold e calibrazione Deep':'fusione live non irreversibile: la finale verrà ricostruita dalle evidenze'}.`:'TSDF mesh non disponibile: la mappa Gaussian resta comunque ricaricabile e ottimizzabile.';
...
async function startPostOptimization(){return startProbabilisticOptimization();}
async function startProbabilisticOptimization(){
  ...
  const current=Math.max(Number(state.probOptimization?.iterations||0),Number(state.liveOptAccepted?.iterations||0)),target=Math.max(current+1,Math.min(CONFIG.probabilisticMaxIterations||120,Math.round(Number($('optIterations')?.value)||Math.max(current+6,CONFIG.probabilisticDefaultIterations||12))));
  ...
  try{
    while(state.postOptBusy&&accepted<target&&attempts<maxAttempts){
      ...
      const shown=state.liveOptStats||state.liveOptCandidateStats,working=state.liveOptWorkingRetained?' · bootstrap interno':'';updateReviewUi();updateOptimizerUi(`OPT UNICO · acc ${accepted}/${target} · tentativi ${attempts} · reproj ${optimizerReprojectionLabel(shown)} · fase ${optimizerPhaseLabel(shown)}${working} · rifiutati run ${state.postOptRejectedRun}`);await new Promise(r=>setTimeout(r,0));
    }
    ...
    if(rebuilt?.withheldReason){state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;window.__ROOMSCAN_METRIC_MESH=null;state.surfaceStats={...(state.surfaceStats||{}),committed:false,committedGaussians:0,committedFaces:0,withheldReason:rebuilt.withheldReason,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:rebuiltMvs};state.renderer?.setData([],{fit:false});state.renderer?.setMesh(null);state.renderer?.draw();log.warn('committed-surface-withheld',{reason:rebuilt.withheldReason,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:rebuiltMvs,rgbEdges:rebuilt.stats?.edgeSwitches||rebuilt.map?.stats?.edgeSwitches||null,alvaEdges:rebuilt.stats?.alvaSwitches||rebuilt.map?.stats?.alvaSwitches||null,reprojectionRobustRmse:rebuilt.stats?.reprojectionRobustRmse??null,advice:'geometry remains diagnostic/candidate-only until global RGB, depth observability and topology checks agree'});}
    if(rebuilt?.map){const committed=rebuilt.map.gaussians||[],mq=rebuilt.map.stats?.meshQuality||null,mvsV=rebuiltMvs,candidateMesh=rebuilt.map.mesh?.vertices?.length?rebuilt.map.mesh:null;state.probOptimized={previewGaussians:geometryReady?committed:[],candidateGaussians:[...(rebuilt.map.candidateGaussians||[]),...(!geometryReady?committed:[])],mapStats:rebuilt.map.stats};if(committed.length&&geometryReady){state.denseCandidateGaussians=[];state.denseCandidateMesh=null;state.gaussians=committed;state.mesh=candidateMesh;window.__ROOMSCAN_METRIC_MESH=state.mesh;state.meshStale=!state.mesh;state.geometryCommitted=true;state.surfaceStats={...(state.surfaceStats||{}),committed:true,committedGaussians:state.gaussians.length,committedFaces:state.mesh?.faces?.length?state.mesh.faces.length/3:0,meshQuality:mq,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,surfaceLayers:rebuilt.map.mesh?.surfaceLayers??null,meshedSurfelFraction:rebuilt.map.mesh?.meshedSurfelFraction??null,eligibleCommittedFrames:rebuilt.map.stats?.eligibleCommittedFrames??null,excludedUnacceptedFrames:rebuilt.map.stats?.excludedUnacceptedFrames??null,mvsValidation:mvsV};state.renderer?.setData(state.gaussians,{fit:false});state.renderer?.setMesh(state.mesh);state.renderer?.draw();if(mvsV&&mvsV.commitFraction<.25)log.warn('committed-mvs-support-low',{mvsValidation:mvsV,advice:'most MVS proposals lack final-pose depth observability; surface is intentionally sparse rather than fabricated'});}else if(committed.length||candidateMesh){state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;window.__ROOMSCAN_METRIC_MESH=null;state.denseCandidateGaussians=committed;state.denseCandidateMesh=candidateMesh;const reason=rebuilt?.withheldReason||rebuiltPolicy?.reason||'final-geometry-policy-rejected';state.surfaceStats={...(state.surfaceStats||{}),committed:false,committedGaussians:0,committedFaces:0,candidateGaussians:committed.length,candidateFaces:candidateMesh?.faces?.length?candidateMesh.faces.length/3:0,withheldReason:reason,meshQuality:mq,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:mvsV};state.renderer?.setData(state.denseCandidateGaussians,{fit:false});state.renderer?.setMesh(state.denseCandidateMesh);state.renderer?.draw();log.warn('diagnostic-candidate-surface-visible',{reason,candidateGaussians:committed.length,candidateFaces:candidateMesh?.faces?.length?candidateMesh.faces.length/3:0,exportAllowed:false,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:mvsV});}else if(!rebuilt?.withheldReason){state.denseCandidateGaussians=[];state.denseCandidateMesh=null;state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;window.__ROOMSCAN_METRIC_MESH=null;const reason=rebuiltPolicy?.reason||'no-dense-evidence-survived-final-pose-validation';state.surfaceStats={...(state.surfaceStats||{}),committed:false,committedGaussians:0,committedFaces:0,withheldReason:reason,meshQuality:mq,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:mvsV};state.renderer?.setData([],{fit:false});state.renderer?.setMesh(null);state.renderer?.draw();log.warn('committed-surface-withheld',{reason,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:mvsV,depthCalibration:rebuilt.map.stats?.depthCalibration||null,rgbEdges:rebuilt.map.stats?.edgeSwitches||null});}}
...
async function loadSavedSession(id){
  ...
  state.denseDepthSamples=bundle.snapshot.denseSamples||bundle.session.counts?.denseSamples||0;state.deepRaySamples=bundle.snapshot.deepRaySamples||0;state.reviewMetricLocked=bundle.snapshot.metricLocked??bundle.session.metricLocked??null;state.reviewKeyframes=bundle.snapshot.keyframes||bundle.session.counts?.keyframes||0;state.lastTracking=null;state.mesh=decodeStoredMesh(bundle.mesh);state.meshStale=!!bundle.mesh?.stale;
  const savedBuild=buildIdOf(bundle.snapshot.build||bundle.session.build),legacyRevalidatable=!!(savedBuild&&savedBuild!==BUILD.id&&state.probGraph?.frames?.length);
  if(legacyRevalidatable){state.denseCandidateGaussians=state.gaussians||[];state.denseCandidateMesh=state.mesh;state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;window.__ROOMSCAN_METRIC_MESH=null;log.warn('legacy-geometry-withheld',{savedBuild,currentBuild:BUILD.id,candidateGaussians:state.denseCandidateGaussians.length,candidateMeshFaces:state.denseCandidateMesh?.faces?.length?state.denseCandidateMesh.faces.length/3:0,reason:'final-pose-dense-revalidation-required'});}else window.__ROOMSCAN_METRIC_MESH=!state.meshStale?state.mesh:null;
  log.info('session-loaded',{id,gaussians:state.gaussians.length,candidateGaussians:state.denseCandidateGaussians?.length||0,optimizerObservations:state.optimizerObservations?.count||0,factorGraph:state.probGraph?.summary?.()||null,iterations:state.optimization.iterations,meshStale:state.meshStale,savedBuild:savedBuild||null});await showReview();
}
