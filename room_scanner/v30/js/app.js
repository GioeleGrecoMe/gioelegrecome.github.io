/**
 * Room Scanner V30.51.0 adaptive Deep + Alva continuity processing.
 *
 * BOOT CONTRACT
 * -------------
...
async function startScan(metric={},epoch=state.bridgeEpoch,bridge=null){
  ...
  state.scanStop=state.camera.loop(frame=>{try{
    state.lastFrameGeometry=frame.geometry;noteFastLaneFrame(frame);const r=state.slam.process(frame),featureState=classifyAlvaFramePoints(r.framePoints||[],Number(frame.at)||performance.now(),r.pose),integrity=updateAlvaTrackingIntegrity(r,frame,featureState);r._integrityAccepted=!!integrity.accept;state.lastTracking=r;drawAlvaFeatureOverlay(featureState,frame.width,frame.height);updateAlvaRecoveryUi(integrity,featureState);
...
function updateAlvaTrackingIntegrity(r,frame,featureState){
  const at=Number(frame?.at)||performance.now(),valid=!!(r?.trackingValid&&r?.pose?.p&&r?.pose?.q),persistent=featureState?.persistent?.length||0,fraction=featureState?.persistentFraction||0,minPersistent=Math.max(4,Number(CONFIG.alvaRecoveryMinPersistent)||18),minFraction=Math.max(.05,Number(CONFIG.alvaRecoveryMinPersistentFraction)||.28),trusted=state.alvaLastTrustedPose;let jump=false,tr=0,rot=0,dt=0;
  ...
  if(state.alvaRecoveryRequired){const returnPoseOk=!trusted||(Math.hypot(r.pose.p[0]-trusted.p[0],r.pose.p[1]-trusted.p[1],r.pose.p[2]-trusted.p[2])<=(Number(CONFIG.alvaRecoveryReturnTranslation)||.9)&&quatAngle(r.pose.q,trusted.q)<=(Number(CONFIG.alvaRecoveryReturnRotationRad)||.9)),featureOk=persistent>=minPersistent&&fraction>=minFraction,explicitReloc=String(r.trackingMode||'').includes('relocalized');if(!jump&&featureOk&&(returnPoseOk||explicitReloc))state.alvaRecoveryStableFrames++;else state.alvaRecoveryStableFrames=0;if(state.alvaRecoveryStableFrames>=(Number(CONFIG.alvaRecoveryStableFrames)||4)){const previous=state.alvaRecoveryReason;state.alvaRecoveryRequired=false;state.alvaRecoveryReason=null;state.alvaRecoveryStableFrames=0;state.alvaLastTrustedPose={p:[...r.pose.p],q:[...r.pose.q]};state.alvaLastTrustedAt=at;log.info('alva-reference-recovered',{frameId:frame?.frameId||null,previousReason:previous,persistentFeatures:persistent,persistentFraction:fraction,trackingMode:r.trackingMode});return {accept:true,recovery:false,recovered:true};}return {accept:false,recovery:true,reason:state.alvaRecoveryReason,jump,translation:tr,rotationRad:rot,returnPoseOk,featureOk};}
...
async function processArchivedDeepAdaptive(records,rgbScaffold=null){
  ...
  for(let round=0;round<Math.max(1,Number(CONFIG.adaptiveDeepMaxRounds)||7);round++){
    ...
    log.info('adaptive-deep-selection',{round,globalUncertainty:plan.map?.globalUncertainty,marginalScore:plan.marginalScore,stopReason:plan.stopReason||null,selected:plan.map?.scores?.filter(x=>plan.records.some(r=>String(r.frameId)===String(x.frameId))).map(x=>({frameId:x.frameId,score:x.score,novelty:x.novelty,localUncertainty:x.localUncertainty,quality:x.quality,featureSupport:x.featureSupport,cell:x.cell}))||[]});
...
async function finishScan(){
  ...
  // V30.51: the large RGB archive builds the scaffold first. Deep is then
  ...
  // V30.51: Processing is responsible for producing the final authoritative
