...
async function refreshPostScanMvsGeometry(payload){
  ...
  try{
    const {buildSparseDepthAnchors}=await lazy('./dense/sparse_depth_anchors.js'),sources=payload.sources.slice(0,Math.max(2,Number(CONFIG.postScanMvsSourcePool)||4)),sparse=buildSparseDepthAnchors(payload.ref,sources,{maxReprojectionPx:CONFIG.denseSeedMaxReprojectionPx||2.8,minAngleRad:CONFIG.denseSeedMinAngleRad||.006,maxGapBaselineRatio:CONFIG.denseSeedMaxGapBaselineRatio||.14});
    payload.sources=sources;payload.sparseSeeds=serializeSparseSeeds(sparse.seeds);
    // IMPORTANT: this range is diagnostic/local only.  All post-scan jobs share
    // one scan-wide Alva-world envelope computed after every view has been
    // rebound and re-triangulated.  Letting each weak view set its own near/far
    // made plane sweep behave as if every image had an independent depth scale.
    payload.localSparseRange=sparse.range?{...sparse.range}:null;
    state.postScanMvsRefreshed++;const out={ok:true,seeds:payload.sparseSeeds.length,matched:sparse.stats?.matched||0,triangulated:sparse.stats?.triangulated||0,localNear:sparse.range?.near??null,localMedian:sparse.range?.median??null,localFar:sparse.range?.far??null,sources:sources.length};log.debug('mvs-postscan-sparse-refreshed',{frameId:payload.ref.frameId,...out});return out;
  }catch(err){state.postScanMvsRefreshFailed++;payload.localSparseRange=null;const out={ok:false,reason:err?.message||String(err)};log.warn('mvs-postscan-sparse-refresh-failed',{frameId:payload?.ref?.frameId||null,...out});return out;}
}
...
async function drainPostScanMvsBacklog(snapshot=null){
  ...
  // Pass 1: re-triangulate EVERY retained view using the final RGB/Alva poses.
  // Only after all local evidence exists can we infer a single shared depth
  // envelope.  This prevents sequential jobs from defining incompatible scales.
  for(let qi=0;qi<queue.length;qi++){const payload=queue[qi];if(!payload?.ref)continue;if(state.processingActive)processingSetPhase('mvs',{index:qi+1,total:queue.length,message:`MVS scala globale ${qi+1}/${queue.length}`});await refreshPostScanMvsGeometry(payload);await new Promise(r=>setTimeout(r,0));}
  const {buildPostScanMvsDepthConsensus,applyPostScanMvsDepthConsensus,cameraTrajectoryDiagonal}=await lazy('./reconstruction/mvs_depth_scale_consensus.js'),metricLocked=!!state.slam?.metricLocked||queue.some(p=>!!p?.ref?.metricLocked),trajectoryDiagonal=cameraTrajectoryDiagonal(queue),scaleConsensus=buildPostScanMvsDepthConsensus(queue,{metricLocked,fallbackNear:metricLocked?(CONFIG.denseNearM||.28):.12,fallbackFar:metricLocked?(CONFIG.denseFarM||8.5):Math.max(5,Number(trajectoryDiagonal||0)*6),trajectoryDiagonal,minReliableSeeds:Number(CONFIG.postScanMvsScaleMinSeeds)||18,minReliableFrames:Number(CONFIG.postScanMvsScaleMinFrames)||2,maxSeedsPerFrame:Number(CONFIG.postScanMvsScaleSeedsPerFrame)||96,optimizerSnapshot:snapshot,minLandmarkSeeds:Number(CONFIG.postScanMvsScaleMinLandmarks)||24,maxLandmarksPerFrame:Number(CONFIG.postScanMvsScaleLandmarksPerFrame)||128});
  for(const payload of queue)applyPostScanMvsDepthConsensus(payload,scaleConsensus);
  log.checkpoint('mvs-postscan-scale-consensus',{mode:scaleConsensus.mode,ready:scaleConsensus.ready,metricLocked,near:scaleConsensus.near,median:scaleConsensus.median,far:scaleConsensus.far,q10:scaleConsensus.q10,q90:scaleConsensus.q90,scaleSource:scaleConsensus.scaleSource||null,reliableSeeds:scaleConsensus.reliableSeeds,reliableFrames:scaleConsensus.reliableFrames,landmarkSeeds:scaleConsensus.landmarkSeeds||0,landmarkFrames:scaleConsensus.landmarkFrames||0,trajectoryDiagonal,localMedians:scaleConsensus.perFrame?.map(x=>({frameId:x.frameId,median:x.median,count:x.count,strict:x.strict}))||[]});
  // Pass 2: all plane sweeps now use exactly the same world-depth interval.
  for(let qi=0;qi<queue.length;qi++){const payload=queue[qi];if(!payload?.ref)continue;if(state.processingActive){const survey=denseKeyframeSurvey(payload.ref);if(survey)showProcessingFrame(survey,qi,queue.length,{phase:'mvs',message:`MVS ${qi+1}/${queue.length} · ${payload.ref.frameId}`});else processingSetPhase('mvs',{index:qi+1,total:queue.length,message:`MVS ${qi+1}/${queue.length}`});}await waitForDenseIdle(20000);const refreshed={ok:true,sharedScale:payload.depthScaleConsensus};payload.jobId=`mvs-post-${++state.denseJobs}`;await dispatchDensePayload(payload);const deadline=performance.now()+Math.max(3000,Number(CONFIG.postScanMvsJobTimeoutMs)||18000);while(state.denseBusy&&performance.now()<deadline)await new Promise(r=>setTimeout(r,30));if(state.denseBusy){timeouts++;state.denseBusy=false;state.denseActivePayload=null;log.warn('mvs-postscan-timeout',{jobId:payload.jobId,frameId:payload.ref.frameId,refreshed});}else processed++;state.stablePhotoProcessingMvsDone=processed;await new Promise(r=>setTimeout(r,0));}
  state.postScanMvsDrainActive=false;state.postScanMvsPayloads=[];const out={processed,timeouts,refreshed:state.postScanMvsRefreshed,refreshFailed:state.postScanMvsRefreshFailed,scaleConsensus:{mode:scaleConsensus.mode,ready:scaleConsensus.ready,near:scaleConsensus.near,median:scaleConsensus.median,far:scaleConsensus.far,scaleSource:scaleConsensus.scaleSource||null,reliableSeeds:scaleConsensus.reliableSeeds,reliableFrames:scaleConsensus.reliableFrames,landmarkSeeds:scaleConsensus.landmarkSeeds||0,landmarkFrames:scaleConsensus.landmarkFrames||0},elapsedMs:performance.now()-started,mvsSamples:state.denseDepthSamples};log.info('mvs-postscan-drain-complete',out);return out;
}
...
async function handleDenseDepthMessage(d){
  ...
  log.debug('mvs-postscan-result',{jobId:d.jobId,frameId:payload?.ref?.frameId||null,samples:d.samples?.length||0,medianDepth:d.medianDepth??null,near:d.near??payload?.near??null,far:d.far??payload?.far??null,boundaryRejected:d.boundaryRejected||0,scaleConsensus:payload?.depthScaleConsensus||null,ms:d.ms??null});
...
function updateReviewUi(){
  ...
  if($('metricGsStats')){const suppressed=!!state.surfaceStats?.candidateMeshDisplaySuppressed,meshReason=state.surfaceStats?.meshWithheldReason||state.surfaceStats?.geometryPolicy?.reason||'topologia non ancora coerente';$('metricGsStats').textContent=state.geometryCommitted&&committedMesh?`GEOMETRIA COMMITTED: ${committedMesh.vertices.length/3} vertici / ${faces} facce · submap fuse dopo scaffold e calibrazione Deep.`:state.geometryCommitted&&state.gaussians?.length?`SPLAT GLOBALE COMMITTED: ${state.gaussians.length} GS · mesh trattenuta (${candidateFaces} facce candidate) · motivo: ${meshReason} · export PLY GS attivo, export mesh disabilitato.`:candidateFaces?`CANDIDATO NON COMMITTED: ${candidateVertices} vertici / ${candidateFaces} facce · ${suppressed?'mesh nascosta perché topologicamente/non-autoritativamente incoerente':'visualizzazione diagnostica soltanto'} · export disabilitato · motivo: ${state.surfaceStats?.withheldReason||state.surfaceStats?.geometryPolicy?.reason||'gate geometrico non superato'}.`:'Nessuna geometria committed. Le evidenze restano salvate e rielaborabili; export 3D disabilitato finché i gate geometrici non sono superati.';}
...
async function startProbabilisticOptimization({automatic=false,additionalPasses=null}={}){
  ...
  try{
    ...
    const rebuiltPolicy=rebuilt?.map?.stats?.geometryPolicy||null,rebuiltDepthPolicy=rebuilt?.map?.stats?.depthGeometryPolicy||null,rebuiltMvs=rebuilt?.map?.stats?.mvsValidation||null;
    const splatReady=(rebuiltPolicy?.splatCommitReady??rebuiltPolicy?.commitReady)!==false&&!rebuilt?.withheldReason,meshReady=splatReady&&(rebuiltPolicy?.meshCommitReady??rebuiltPolicy?.commitReady)!==false&&!rebuilt?.meshWithheldReason;
    ...
    if(rebuilt?.map){
      const committed=rebuilt.map.gaussians||[],mq=rebuilt.map.stats?.meshQuality||null,mvsV=rebuiltMvs,diagnosticMesh=rebuilt.map.mesh?.vertices?.length?rebuilt.map.mesh:null;
      state.probOptimized={previewGaussians:splatReady?committed:[],candidateGaussians:[...(rebuilt.map.candidateGaussians||[]),...(!splatReady?committed:[])],mapStats:rebuilt.map.stats};
      if(committed.length&&splatReady){
        state.gaussians=committed;state.denseCandidateGaussians=[];state.mesh=meshReady?diagnosticMesh:null;state.denseCandidateMesh=meshReady?null:diagnosticMesh;window.__ROOMSCAN_METRIC_MESH=state.mesh;state.meshStale=!state.mesh;state.geometryCommitted=true;
        const meshWithheldReason=!meshReady?(rebuilt?.meshWithheldReason||rebuiltPolicy?.reason||'mesh-policy-rejected'):null;
        state.surfaceStats={...(state.surfaceStats||{}),committed:true,committedGaussians:state.gaussians.length,committedFaces:state.mesh?.faces?.length?state.mesh.faces.length/3:0,candidateFaces:!meshReady&&diagnosticMesh?.faces?.length?diagnosticMesh.faces.length/3:0,candidateMeshDisplaySuppressed:!meshReady&&!!diagnosticMesh,meshWithheldReason,meshQuality:mq,rawMeshQuality:rebuilt.map.stats?.rawMeshQuality||null,meshCleanup:rebuilt.map.stats?.meshCleanup||null,globalSurfaceConsensus:rebuilt.map.stats?.globalSurfaceConsensus||null,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,surfaceLayers:diagnosticMesh?.surfaceLayers??null,meshedSurfelFraction:diagnosticMesh?.meshedSurfelFraction??null,eligibleCommittedFrames:rebuilt.map.stats?.eligibleCommittedFrames??null,excludedUnacceptedFrames:rebuilt.map.stats?.excludedUnacceptedFrames??null,mvsValidation:mvsV};
        const visibleCommitted=surfaceGaussiansForDisplay(state.gaussians,{mode:'review',source:'review-committed-rebuild'});state.renderer?.setData(visibleCommitted,{fit:false});state.renderer?.setMesh(state.mesh);state.renderer?.draw();
        if(meshWithheldReason)log.warn('committed-splat-mesh-withheld',{reason:meshWithheldReason,committedGaussians:state.gaussians.length,candidateFaces:diagnosticMesh?.faces?.length?diagnosticMesh.faces.length/3:0,geometryPolicy:rebuiltPolicy});
        if(mvsV&&mvsV.commitFraction<.25)log.warn('committed-mvs-support-low',{mvsValidation:mvsV,advice:'most MVS proposals lack final-pose depth observability; surface is intentionally sparse rather than fabricated'});
      }else if(!rebuilt?.withheldReason){
