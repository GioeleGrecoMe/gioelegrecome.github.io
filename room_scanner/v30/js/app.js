/**
 * Room Scanner V30.30.0 live pose-aware photo/depth puzzle application.
 *
 * BOOT CONTRACT
 * -------------
...
async function handleDeepDepthMessage(d){
  ...
  if(binding.kind==='preview'){
    ...
    // estimates metric scale from RGB correspondences + exact Alva poses; a
...
async function persistCurrentSession({status='finished',keyframes=state.reviewKeyframes||0}={}){
  ...
  const snapshot={id,sessionId:id,format:'ROOMSCAN-PUZZLE-SESSION-4',savedAt:Date.now(),build:BUILD.id,gaussians:state.gaussians,optimizerObservations:state.optimizerObservations||null,factorGraph:graphState,deepSequence,probOptimization:state.probOptimization||null,puzzleReconstruction:state.puzzleReconstruction||null,coverageSummary:coverageSnapshot(),optimization:{...(state.optimization||{iterations:0})},denseSamples:state.denseDepthSamples||0,deepRaySamples:state.deepRaySamples||0,metricLocked:state.reviewMetricLocked??!!state.slam?.metricLocked,keyframes};
...
function updateLiveMapUi(extra=null){
  ...
  if(status){if(extra)status.textContent=extra;else if(!s.frames)status.textContent=`${state.liveMapMode==='depth'?'GLOBAL DEPTH':'PHOTO PUZZLE'} · attendo prima foto Deep con posa Alva…`;else{const depthErr=Number.isFinite(s.depthScaleError)&&s.depthScaleError<9?` · σscale ${(100*s.depthScaleError).toFixed(1)}%`:'';status.textContent=`${state.liveMapMode==='depth'?'GLOBAL DEPTH':'PHOTO PUZZLE'} · ${s.connectedFrames||0}/${s.frames||0} foto · ${s.edges||0} link${s.loops?` · ${s.loops} loop`:''} · Deep ${s.rawDepthFrames||0}F → metriche ${s.metricDepthFrames||0}F${depthErr} · ${(100*(s.coverage||0)).toFixed(0)}% atlas`;}}
}
