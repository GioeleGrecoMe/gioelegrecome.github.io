/**
 * Room Scanner V30.47.0 canonical RGB scaffold + post-scan MVS/Deep application.
 *
 * BOOT CONTRACT
 * -------------
...
async function beginBridge(){
  // V30.47.0: only ProbabilisticJointOptimizer is operational; RGB/Alva own the scan clock and dense work is post-scan.
...
function requestLiveDeepPreview(frame,tracking){
  ...
  const now=Number(frame.at)||performance.now(),planGate=lateDeepPlanGate(now);if(!planGate.ok)return;
  const qstats=state.deepLateQueue?.stats?.()||{};if((qstats.surveys||0)>=(CONFIG.deepSurveyQueueBudget||2))return;
...
async function drainPostScanMvsBacklog(snapshot=null){
  ...
  for(const payload of queue){if(!payload?.ref)continue;await waitForDenseIdle(20000);const refreshed=await refreshPostScanMvsGeometry(payload);payload.jobId=`mvs-post-${++state.denseJobs}`;await dispatchDensePayload(payload);const deadline=performance.now()+Math.max(3000,Number(CONFIG.postScanMvsJobTimeoutMs)||18000);while(state.denseBusy&&performance.now()<deadline)await new Promise(r=>setTimeout(r,30));if(state.denseBusy){timeouts++;state.denseBusy=false;state.denseActivePayload=null;log.warn('mvs-postscan-timeout',{jobId:payload.jobId,frameId:payload.ref.frameId,refreshed});}else processed++;await new Promise(r=>setTimeout(r,0));}
...
async function showReview(){
  show('review');const {GaussianRenderer}=await lazy('./gaussian/renderer.js');if(!state.renderer)state.renderer=new GaussianRenderer($('viewer'));state.renderer.setData(committedGeometryAvailable()?state.gaussians:[],{fit:true});state.renderer.setMesh(committedMeshAvailable()?state.mesh:null);state.renderer.draw();updateReviewUi();updateOptimizerUi();
}
function updateReviewUi(){
  ...
  if($('reviewStats'))$('reviewStats').textContent=`Surface splat: ${committedGeometryAvailable()?state.gaussians.length:0}${candidateGaussians?` · ${candidateGaussians} candidati non committed`:''} · depth sample ${state.denseDepthSamples||0} · scala ${metric?'metrica':'Alva libera'} · keyframe ${kf} · ottimizzazione ${iterations} iterazioni`;
  if($('metricGsStats'))$('metricGsStats').textContent=state.geometryCommitted&&committedMesh?`GEOMETRIA COMMITTED: ${committedMesh.vertices.length/3} vertici / ${faces} facce · submap fuse dopo scaffold e calibrazione Deep.`:candidateFaces?`CANDIDATO NON COMMITTED: ${candidateVertices} vertici / ${candidateFaces} facce · visualizzazione diagnostica soltanto, export disabilitato · motivo: ${state.surfaceStats?.withheldReason||state.surfaceStats?.geometryPolicy?.reason||'gate geometrico non superato'}.`:'Nessuna geometria committed. Le evidenze restano salvate e rielaborabili; export 3D disabilitato finché i gate geometrici non sono superati.';
...
async function startProbabilisticOptimization(){
  ...
  try{
    ...
    if(rebuilt?.withheldReason){const diagnosticCandidate=rebuilt?.map?.gaussians||[],diagnosticMesh=rebuilt?.map?.mesh?.vertices?.length?rebuilt.map.mesh:null;state.denseCandidateGaussians=diagnosticCandidate;state.denseCandidateMesh=diagnosticMesh;state.gaussians=[];state.mesh=null;state.meshStale=true;state.geometryCommitted=false;window.__ROOMSCAN_METRIC_MESH=null;state.surfaceStats={...(state.surfaceStats||{}),committed:false,committedGaussians:0,committedFaces:0,candidateGaussians:diagnosticCandidate.length,candidateFaces:diagnosticMesh?.faces?.length?diagnosticMesh.faces.length/3:0,withheldReason:rebuilt.withheldReason,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:rebuiltMvs};state.renderer?.setData(diagnosticCandidate,{fit:false});state.renderer?.setMesh(diagnosticMesh);state.renderer?.draw();if(diagnosticCandidate.length||diagnosticMesh)updateOptimizerUi(`CANDIDATO NON COMMITTED · ${diagnosticCandidate.length} GS${diagnosticMesh?.faces?.length?` · ${diagnosticMesh.faces.length/3} facce`:''} · export disabilitato · ${rebuilt.withheldReason}`);log.warn('diagnostic-candidate-surface-visible',{reason:rebuilt.withheldReason,candidateGaussians:diagnosticCandidate.length,candidateFaces:diagnosticMesh?.faces?.length?diagnosticMesh.faces.length/3:0,exportDisabled:true,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:rebuiltMvs});log.warn('committed-surface-withheld',{reason:rebuilt.withheldReason,geometryPolicy:rebuiltPolicy,depthGeometryPolicy:rebuiltDepthPolicy,mvsValidation:rebuiltMvs,rgbEdges:rebuilt.stats?.edgeSwitches||rebuilt.map?.stats?.edgeSwitches||null,alvaEdges:rebuilt.stats?.alvaSwitches||rebuilt.map?.stats?.alvaSwitches||null,reprojectionRobustRmse:rebuilt.stats?.reprojectionRobustRmse??null,advice:'geometry remains diagnostic/candidate-only until global RGB, depth observability and topology checks agree'});}
