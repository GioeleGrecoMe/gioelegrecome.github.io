...
export function analyzeMeshQuality(mesh={}){
  ...
  // dominant component.
  const fragmented=faceCount>=20&&componentCount>=8&&fraction<.35,coherent=faceCount>=20&&fraction>=.55;
  return {vertexCount,faceCount,usedVertices:usedCount,unusedVertices:vertexCount-usedCount,componentCount,componentSizes:components.slice(0,24),largestComponentVertices:largest,largestComponentFraction:fraction,degenerateFaces:degenerate,bbox:extent?{min:mins,max:maxs,extent,diagonal:diag}:null,edgeMedian:Number.isFinite(edgeMedian)?edgeMedian:null,edgeP90:Number.isFinite(edgeP90)?edgeP90:null,fragmentationScore:clamp(1-fraction,0,1),status:fragmented?'fragmented':coherent?'coherent':'partial'};
}
