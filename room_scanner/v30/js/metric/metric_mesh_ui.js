/*
 * V30.14 metric GS/mesh diagnostics UI.
 *
...
export function installMetricMeshUi(){
  ...
  if(review){
    ...
    let meshButton=$('buildMetricMeshBtn');
    if(actions&&!meshButton){meshButton=document.createElement('button');meshButton.id='buildMetricMeshBtn';meshButton.textContent='Crea mesh metrica GS';actions.appendChild(meshButton);}
    if(meshButton&&meshButton.dataset.metricMeshBound!=='1'){
      meshButton.dataset.metricMeshBound='1';
      meshButton.addEventListener('click',async()=>{try{const mesh=meshState.mesh||await prepareReviewMesh({force:true});if(mesh)downloadPly(mesh);}catch(err){alert(`Meshing fallito: ${err.message}`);}});
    }
...
export function updateMetricMeshUi(){
  ...
  if(d){
    if(!s?.gaussians)d.textContent='Dati mesh: nessuno snapshot GS disponibile.';
      ...
      const unit=s.metricLocked?'m':'u.Alva';d.textContent=`GS: ${s.bounds?.count||0} · campioni: ${s.samples?.length||0} · bbox: ${s.bounds?.size?.map(v=>v.toFixed(3)).join(' × ')} ${unit} · scala: ${s.metricLocked?'METRICA':'ALVA LIBERA'} · mesh: ${meshState.busy?'calcolo…':m?`${nv} vertici / ${nf} facce`:'attesa'}`;
    }
  }
...
export async function prepareReviewMesh({force=false}={}){
  ...
  // Meshing is valid in Alva's internally consistent world even when the
  // absolute unit is not yet metres. A later Sim(3) scale can metricise it.
...
export async function requestLiveMesh({minIntervalMs=5500,minSamples=260}={}){
  ...
  if(!s?.samples?.length||s.samples.length<minSamples||meshState.busy||now-(meshState.lastLiveAt||0)<minIntervalMs)return meshState.mesh;
