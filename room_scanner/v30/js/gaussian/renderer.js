...
function shader(gl,type,src){const s=gl.createShader(type);gl.shaderSource(s,src);gl.compileShader(s);if(!gl.getShaderParameter(s,gl.COMPILE_STATUS))throw new Error(gl.getShaderInfoLog(s));return s;}function program(gl){const p=gl.createProgram();gl.attachShader(p,shader(gl,gl.VERTEX_SHADER,VERT));gl.attachShader(p,shader(gl,gl.FRAGMENT_SHADER,FRAG));gl.linkProgram(p);if(!gl.getProgramParameter(p,gl.LINK_STATUS))throw new Error(gl.getProgramInfoLog(p));return p;}
...
export class GaussianRenderer{
 ...
 render(){this._resize();if(this.fallback)return this._render2d();const gl=this.gl;gl.viewport(0,0,this.canvas.width,this.canvas.height);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);if(!this.count)return;const cp=Math.cos(this.pitch),eye=[this.target[0]+this.pan[0]+this.distance*cp*Math.sin(this.yaw),this.target[1]+this.pan[1]+this.distance*Math.sin(this.pitch),this.target[2]+this.pan[2]+this.distance*cp*Math.cos(this.yaw)],target=[this.target[0]+this.pan[0],this.target[1]+this.pan[1],this.target[2]+this.pan[2]];gl.useProgram(this.prog);gl.uniformMatrix4fv(this.uView,false,lookAt(eye,target));gl.uniformMatrix4fv(this.uProj,false,mat4Perspective(Math.PI/3,this.canvas.width/this.canvas.height,.03,100));gl.uniform1f(this.uPointScale,this.pointScale);gl.bindVertexArray(this.vao);gl.depthMask(false);gl.drawArraysInstanced(gl.TRIANGLES,0,6,this.count);gl.depthMask(true);gl.bindVertexArray(null);}
 _render2d(){const c=this.ctx;c.fillStyle='#07090d';c.fillRect(0,0,this.canvas.width,this.canvas.height);if(!this._arrays)return;c.fillStyle='#ddd';const p=this._arrays.center,n=this.count,scale=Math.min(this.canvas.width,this.canvas.height)/(this.distance*2.2);for(let i=0;i<n;i+=Math.max(1,Math.floor(n/25000))){const j=i*3,x=this.canvas.width/2+(p[j]-this.target[0])*scale,y=this.canvas.height/2-(p[j+2]-this.target[2])*scale;c.fillRect(x,y,1.5,1.5);}}
 ...
 fit(){this._fitDone=false;if(this._arrays)this.setData({data:this._packCurrent(),stride:16});}
