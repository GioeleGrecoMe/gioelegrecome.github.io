...
export class GaussianRenderer{
 ...
 _render2d(){const c=this.ctx;if(!c)return;c.fillStyle='#07090d';c.fillRect(0,0,this.canvas.width,this.canvas.height);if(!this._arrays)return;c.fillStyle='#ddd';const p=this._arrays.center,n=this.count,scale=Math.min(this.canvas.width,this.canvas.height)/(this.distance*2.2);for(let i=0;i<n;i+=Math.max(1,Math.floor(n/25000))){const j=i*3,x=this.canvas.width/2+(p[j]-this.target[0])*scale,y=this.canvas.height/2-(p[j+2]-this.target[2])*scale;c.fillRect(x,y,1.5,1.5);}}
