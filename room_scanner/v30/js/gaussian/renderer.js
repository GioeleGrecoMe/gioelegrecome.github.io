...
export class GaussianRenderer{
  ...
  draw(){
    ...
    const pts=[];for(const s of this.items){const p=this._view(s.p,w,h);if(!p)continue;const e=projectReviewCovariance(s.cov,p,this.yaw,this.pitch,this.splat),rx=e?.rx??Math.max(.55,Math.min(20,(s.scale[0]||s.r||.02)*p.f/p.z*6.5*this.splat)),ry=e?.ry??Math.max(.55,Math.min(20,(s.scale[1]||s.r||.02)*p.f/p.z*6.5*this.splat));pts.push({...p,rx,ry,angle:e?.angle||0,c:s.c,a:s.a,q:s.q});}pts.sort((a,b)=>b.z-a.z);
    for(const p of pts){g.globalAlpha=Math.max(.05,Math.min(.9,p.a*(.45+.55*p.q)));g.fillStyle=`rgb(${p.c[0]},${p.c[1]},${p.c[2]})`;g.beginPath();g.ellipse(p.x,p.y,p.rx,p.ry,p.angle,0,Math.PI*2);g.fill();}g.globalAlpha=1;this._drawAxes(g,w,h);
  }
...
function normalize(items){if(!Array.isArray(items))return [];return items.map(g=>{const p=g.position||g.p||g.mean||g.xyz||[0,0,0],c=g.color||g.rgb||[180,210,240],scale=g.scale||g.scales||[g.radius||.02,g.radius||.02,g.radius||.02],S=Array.isArray(scale)?scale.map(Number):[Number(scale),Number(scale),Number(scale)],r=Math.max(...S),cov=Array.isArray(g.covariance)&&g.covariance.length>=6?g.covariance.slice(0,6).map(Number):null;return {p:p.map(Number),c:c.map(v=>Math.max(0,Math.min(255,Number(v)||0))),scale:S.map(v=>Number.isFinite(v)&&v>0?v:.02),cov:cov?.every(Number.isFinite)?cov:null,r:Number.isFinite(r)?r:.02,a:Number(g.opacity??g.alpha??1),q:Number(g.confidence??.6)}}).filter(g=>g.p.every(Number.isFinite));}

function projectReviewCovariance(C,p,yaw,pitch,splat){if(!Array.isArray(C)||C.length<6||!C.slice(0,6).every(Number.isFinite))return null;const cy=Math.cos(yaw),sy=Math.sin(yaw),cp=Math.cos(pitch),sp=Math.sin(pitch),R=[cy,0,-sy,-sp*sy,cp,-sp*cy,cp*sy,sp,cp*cy],x=(p.x-(0)),z=p.z;
  // p.x is already screen space; recover camera x from perspective relation.
  // The derivative can be built from x_cam/z = (screen-center)/f, but the
  // screen center is unavailable here. _view now exposes xCam/yCam below.
  if(!Number.isFinite(p.xCam+p.yCam+p.z))return null;const jx=[p.f/p.z,0,-p.f*p.xCam/(p.z*p.z)],jy=[0,-p.f/p.z,p.f*p.yCam/(p.z*p.z)],wx=rowToWorld(jx,R),wy=rowToWorld(jy,R),a=Math.max(1e-8,quad(C,wx)),b=bilinear(C,wx,wy),c=Math.max(1e-8,quad(C,wy)),tr=(a+c)*.5,d=Math.sqrt(Math.max(0,((a-c)*.5)**2+b*b)),l1=tr+d,l2=Math.max(1e-8,tr-d),gain=2*Math.max(.25,Number(splat)||1);return {rx:Math.max(.55,Math.min(24,gain*Math.sqrt(l1))),ry:Math.max(.55,Math.min(24,gain*Math.sqrt(l2))),angle:.5*Math.atan2(2*b,a-c)};}
function rowToWorld(j,R){return [j[0]*R[0]+j[1]*R[3]+j[2]*R[6],j[0]*R[1]+j[1]*R[4]+j[2]*R[7],j[0]*R[2]+j[1]*R[5]+j[2]*R[8]];}function quad(C,v){return v[0]*(C[0]*v[0]+C[1]*v[1]+C[2]*v[2])+v[1]*(C[1]*v[0]+C[3]*v[1]+C[4]*v[2])+v[2]*(C[2]*v[0]+C[4]*v[1]+C[5]*v[2]);}function bilinear(C,a,b){return a[0]*(C[0]*b[0]+C[1]*b[1]+C[2]*b[2])+a[1]*(C[1]*b[0]+C[3]*b[1]+C[4]*b[2])+a[2]*(C[2]*b[0]+C[4]*b[1]+C[5]*b[2]);}
