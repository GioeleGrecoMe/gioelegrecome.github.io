...
export class LiveReconstructionOverlay{
  constructor(canvas,{maxSplats=3200}={}){this.canvas=canvas;this.ctx=canvas.getContext('2d');this.maxSplats=maxSplats;this.gaussians=[];this.mesh=null;this.mode='gs';this.referencePoint=null;this._lastSize='';this.canvas.style.pointerEvents='none';this.canvas.style.zIndex='2';}
  ...
  draw({pose,K,geometry,video,framePoints=[]}={}){
    ...
    const g=this.ctx;g.clearRect(0,0,w,h);this._drawAlvaPoints(g,{framePoints,geometry,w,h});if(this.mode==='gs'||this.mode==='both')this._drawGaussians(g,{pose,K,geometry,w,h});if((this.mode==='mesh'||this.mode==='both')&&this.mesh)this._drawMesh(g,{pose,K,geometry,w,h});this._drawReference(g,{pose,K,geometry,w,h});
  }
