...
test('live and review use one in-process ProbabilisticJointOptimizer runtime with accepted/working split',()=>{
  ...
  assert.match(app,/SingleOptimizerRuntime/);assert.match(app,/single-opt-cycle-dispatch/);assert.match(app,/single-opt-candidate-rejected/);assert.match(app,/async function startPostOptimization\(\)\{return startProbabilisticOptimization\(\{automatic:false\}\);\}/);
