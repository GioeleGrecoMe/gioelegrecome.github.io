...
test('post-scan estimator runs RGB pose loop faster than Deep feedback loop',()=>{
  const frames=[0,.1].map((x,i)=>({frameId:`q${i}`,posePrior:pose(x),poseEstimate:pose(x),poseCov:{diag:[1e-4,1e-4,1e-4,1e-5,1e-5,1e-5]},K,width:160,height:120})),graph={format:'ROOMSCAN-PROB-GRAPH-1',frames,edgeFactors:[],alvaFactors:[],landmarkFactors:[],deepFactors:[],mvsFactors:[]},opt=new ProbabilisticJointOptimizer(graph,{depthFeedbackEvery:2});opt.step(1);assert.equal(opt.lastStats.feedbackPhase,'depth-feedback');opt.step(1);assert.equal(opt.lastStats.feedbackPhase,'rgb-fast');opt.step(1);assert.equal(opt.lastStats.feedbackPhase,'depth-feedback');
});
