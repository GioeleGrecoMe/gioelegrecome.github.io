...
const html=fs.readFileSync(path.join(root,'room_scanner_v30.html'),'utf8'),app=fs.readFileSync(path.join(root,'js/app.js'),'utf8');for(const id of ['cameraHeight','startBtn','loadPlyBtn','loadR30Btn','diagDownloadBtn','selfTestBtn','viewer','liveMesh','processDeepBtn','exportMeshBtn','statMesh'])if(!html.includes(`id="${id}"`))throw new Error(`HTML missing #${id}`);
if(!html.includes('js/app.js'))throw new Error('app module not loaded');
if(!app.includes('startVisualMeshRebuild')||!app.includes('integrate-visual'))throw new Error('visual mesh controls missing');
