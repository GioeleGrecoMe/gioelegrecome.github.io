...
test('minimal boot imports the versioned core and depth diagnostic',()=>{const b=read('js/boot.js');for(const m of ['app.js','deep_live_controller.js','deep_diagnostic_controller.js'])assert.match(b,new RegExp(`import\\('\\./${m.replace('.', '\\.')}\\?v=30.18.9'\\)`));for(const x of ['boot_preflight','V30Database','serviceWorker.register','gaussian_metric_tap'])assert.doesNotMatch(b,new RegExp(x));});
test('HTML registers a normal service worker without a stale-controller gate',()=>{const h=read('room_scanner_v30.html');assert.match(h,/register\('sw\.js',\{scope:'\.\/',updateViaCache:'none'\}\)/);assert.match(h,/controllerchange/);assert.match(h,/loadModule\(\)/);for(const stale of ['roomscan-v30-sw-clean-attempt','new MessageChannel','GET_VERSION','getRegistrations'])assert.doesNotMatch(h,new RegExp(stale));const s=read('sw.js');assert.match(s,/const VERSION = '30\.18\.9'/);assert.match(s,/event\.data\?\.type === 'GET_VERSION'/);});
