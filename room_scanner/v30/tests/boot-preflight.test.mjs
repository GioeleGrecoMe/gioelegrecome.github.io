...
test('minimal boot imports only the versioned core app',()=>{const b=read('js/boot.js');assert.match(b,/import\('\.\/app\.js\?v=30.11.4'\)/);for(const x of ['boot_preflight','V30Database','serviceWorker.register','gaussian_metric_tap'])assert.doesNotMatch(b,new RegExp(x));});
test('HTML handshakes with active service worker and removes stale V30 workers before modules',()=>{const h=read('room_scanner_v30.html');assert.match(h,/GET_VERSION/);assert.match(h,/new MessageChannel\(\)/);assert.match(h,/getRegistrations\(\)/);assert.match(h,/roomscan-v30-sw-clean-attempt/);assert.match(h,/k\.startsWith\('room-scanner-v30'\)/);assert.match(h,/loadModule\(\)/);const s=read('sw.js');assert.match(s,/const VERSION='30.11.4'/);assert.match(s,/event\.data\?\.type==='GET_VERSION'/);});
