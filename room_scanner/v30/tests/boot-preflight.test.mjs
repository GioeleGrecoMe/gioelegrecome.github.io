...
test('minimal boot imports core first and keeps diagnostics optional',()=>{
  ...
  assert.match(b,/import\('\.\/app\.js\?v=30\.27\.0-exp\.4'\)/);
  assert.match(b,/import\('\.\/deep_diagnostic_controller\.js\?v=30\.27\.0-exp\.4'\)/);
...
test('service worker activates without a blocking precache and is network-first',()=>{
  const s=read('sw.js');
  assert.match(s,/const VERSION = '30\.27\.0-exp\.4'/);
