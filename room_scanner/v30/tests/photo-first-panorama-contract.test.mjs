...
test('RGB+Deep admission survives missing Alva tracking and detects features on the frozen photo',()=>{
  assert.match(app,/commitCameraFrameWithRelativeDepth\?\.\(survey/);assert.match(app,/const hasAlva=!!\(tracking\?\.trackingValid/);assert.doesNotMatch(app,/if\(!state\.liveMap\|\|!state\.scanK\|\|!tracking\?\.trackingValid/);assert.match(live,/detectPhotoFeatures\(gray,w,h/);assert.match(live,/pose:clonePoseNullable/);assert.match(live,/only a frame with depth evidence may enter the RGB/);
});
