...
test('visual reference sidecar recovers a posed landmark view without becoming a trajectory source',()=>{
  ...
  assert.equal(out.ok,true,out.reason);assert.equal(out.candidateFrameId,'known');assert.ok(out.inliers>=8,out.inliers);assert.ok(out.rmsePx<1,out.rmsePx);assert.ok(out.observations.length>=8,'post-scan recovery needs the verified landmark correspondences');assert.equal(relocalizationPoseCompatible(truth,out,{maxTranslation:.2,maxRotationRad:.2}),true);assert.equal(relocalizationPoseCompatible({p:[4,0,0],q:[0,0,0,1]},out,{maxTranslation:.2,maxRotationRad:.2}),false);
});
