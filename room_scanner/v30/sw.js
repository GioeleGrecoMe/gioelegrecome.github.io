...
self.addEventListener('fetch',event=>{const req=event.request,url=new URL(req.url);if(req.method!=='GET')return;
  // AlvaAR is too large to make the shell install depend on a third-party
  // network request. If the upstream CDN fallback is used successfully once,
  // cache that CORS module so subsequent scans can reuse it offline.
  if(url.hostname==='cdn.jsdelivr.net'&&/\/gh\/alanross\/AlvaAR(?:@[^/]+)?\/dist\/alva_ar\.js$/.test(url.pathname)){event.respondWith((async()=>{const cache=await caches.open(CACHE),hit=await cache.match(req);if(hit)return hit;const fresh=await fetch(req);if(fresh.ok)await cache.put(req,fresh.clone());return fresh;})());return;}
