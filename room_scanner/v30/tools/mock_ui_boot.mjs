...
class El extends EventTarget{constructor(id,cls=''){super();this.id=id;this.classList=new Classes(cls);this.dataset={};this.attributes={};this.disabled=false;this.hidden=false;this.textContent='';this.value='';this.files=[];this.style={};this.open=false;}setAttribute(k,v){this.attributes[k]=String(v);}getAttribute(k){return this.attributes[k]??null;}click(){this.dispatchEvent(new Event('click'));}getBoundingClientRect(){return {width:390,height:844};}getContext(){return {setTransform(){},clearRect(){},beginPath(){},arc(){},moveTo(){},lineTo(){},stroke(){},fill(){},fillText(){},set strokeStyle(v){},set fillStyle(v){},set lineWidth(v){},set font(v){}};}}
const ids=['home','calibration','bridge','scan','review','calibrateBtn','clearCalibrationBtn','calibSummary','startBtn','filePly','fileR30','homeStatus','savedSessions','buildBadge','buildFoot','calibOverlay','calibStatus','calibDepth','calibAddPinBtn','calibUndoPinBtn','calibFinishBtn','calibCancelBtn','bridgeCamera','bridgeCoach','bridgeFound','bridgeInliers','bridgeRmse','bridgePinGuidance','bridgePinInstructions','bridgeRetryBtn','bridgeCancelBtn','camera','miniMap','scanDiagnosticsToggle','scanDiagnostics','scanDiagnosticsClose','scanExportDiagBtn','liveOptimizerHud','liveMapState','liveMapPhotoBtn','liveMapDepthBtn','liveMapCanvas','slamState','alvaPtsState','metricState','mvsState','statFeat','statMatch','statKf','statGs','statTri','metricPipelineHud','coach','arModeBtn','pinBtn','finishBtn','viewer','backHomeBtn','fitBtn','viewTopBtn','viewFrontBtn','viewSideBtn','splatSize','reviewStats','metricGsStats','optIterations','optStartBtn','optStopBtn','optProgress','optStatus','coverageSphere','coverageState','buildMetricMeshBtn','exportPlyBtn','exportR30Btn','exportDiagBtn','resumeBtn','loadPlyBtn','loadR30Btn','diagPanel','diagLive','diagDownloadBtn','diagCopyBtn','selfTestBtn','forceUpdateBtn','diagForceUpdateBtn','selfTestSummary'];
for(const id of ['chooseDeepModelBtn','deepModelFile','testDeepBtn','deepModelStatus','deepTestPreview','depthOverlay'])if(!ids.includes(id))ids.push(id);
...
if(els.get('buildBadge').textContent!=='V30.40.0')throw new Error('wrong badge');
...
// V30.40.0 regression: the single-optimizer cleanup must not leave calls to
