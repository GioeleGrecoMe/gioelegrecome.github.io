...
const V11_WORKER_BUILD = 'room-scanner-v11-d5-depth-worker-boottrace-2026-08-16';
...
async function boot() {
  try {
    ...
    // D5: WebGPU diagnostics must never block loading the Depth worker/model.
    ...
    // only the code payload; self.location remains depth_ai_worker_v11_d5.js, so same-directory
    // relative asset resolution remains correct.
    const blob = new Blob([patched.src + `\n//# sourceURL=depth_ai_worker_v11_d5_runtime.js\n`], { type: 'text/javascript' });
