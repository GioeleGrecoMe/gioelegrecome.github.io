# Room Scanner V14.0.1 — Room Cells
...
## V14.0.1 — synchronized WebXR panorama capture

V14.0.1 fixes a frame-lifetime regression in V14.0.0. The panorama remains a slow manual sweep using the live WebXR raw camera, exactly as intended. Pressing **Scatta foto** no longer accesses `XRFrame` or `XRWebGLBinding.getCameraImage()` from the DOM click callback. The click only queues a request; the next valid `XRSession.requestAnimationFrame()` callback copies RGB, pose, projection and optional CPU depth from the same frame and resolves the photo request.

This preserves strict synchronization between the image and its metric camera pose while respecting the WebXR lifetime of `XRFrame`/`XRCamera`. A 3.5 s timeout, double-click guard, tracking-loss guard and session-end rejection prevent stuck capture state.
