 ...
 */
'use strict';
const DIAG_BUILD='rsdiag-20260815-4';
...
;(()=>{
  ...
  const bridge={version:'rsdiag-bridge-20260815-2',snapshot:()=>({
    ...
    objectSeed:safe(()=>({active:S.objectSeeding?.active,busy:S.objectSeeding?.busy,navBusy:S.objectSeeding?.navBusy,unavailable:S.objectSeeding?.unavailable,snapshotFrameId:S.objectSeeding?.snapshotFrameId,freezeActive:S.objectSeeding?.freezeActive,readiness:S.objectSeeding?.readiness,promptUV:S.objectSeeding?.promptUV,roiUV:S.objectSeeding?.roiUV,inferenceProgress:S.objectSeeding?.inferenceProgress,pending:S.objectSeeding?.pending?{frameId:S.objectSeeding.pending.F?.id,backend:S.objectSeeding.pending.backend,candidate:{iou:S.objectSeeding.pending.candidate?.iou,confidence:S.objectSeeding.pending.candidate?.confidence,metricReady:S.objectSeeding.pending.candidate?.metricReady,points:S.objectSeeding.pending.candidate?.pts?.length,directDepthPoints:S.objectSeeding.pending.candidate?.directDepthPoints,projectedSurfelPoints:S.objectSeeding.pending.candidate?.projectedSurfelPoints,depthAnything:S.objectSeeding.pending.candidate?.depthAnything}}:null,currentCapture:S.objectSeeding?.currentCapture?{views:S.objectSeeding.currentCapture.views?.length,name:S.objectSeeding.currentCapture.name}:null})),
    depthAI:safe(()=>({enabled:S.depthAI?.enabled,status:S.depthAI?.status,provider:S.depthAI?.provider,modelSource:S.depthAI?.modelSource,runtimeSource:S.depthAI?.runtimeSource,lastError:S.depthAI?.lastError,lastInferenceMs:S.depthAI?.lastInferenceMs,fusedFrames:S.depthAI?.fusedFrames,fusedPoints:S.depthAI?.fusedPoints,rejectedFrames:S.depthAI?.rejectedFrames,liveRuns:S.depthAI?.liveRuns,keyframes:S.depthAI?.keyframes?.length,alignment:S.depthAI?.alignment?.slice(-8)})),
