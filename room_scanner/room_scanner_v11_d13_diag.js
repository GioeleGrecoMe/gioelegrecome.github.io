(() => {
  'use strict';
  const BUILD='room-scanner-v11-d13-depth-boottrace-2026-08-16';
  const pageEvents=[];
  const MAX_EVENTS=80;
  const RX=/depth|onnx|ort\.|wasm|webgpu|inference|session/i;
  let previousTrace=null;try{const raw=localStorage.getItem('roomScannerV11D13LastTrace');if(raw)previousTrace=JSON.parse(raw)}catch(_){}
  function push(type,detail){
    ...
    try{localStorage.setItem('roomScannerV11D13LastTrace',JSON.stringify({build:BUILD,updatedAt:new Date().toISOString(),href:location.href,pageEvents:pageEvents.slice(-80)}))}catch(_){}
    updateCompactStatus();
    updateLiveTrace();
  }
  window.__DEPTH_V11_D13_PUSH__=(type,detail)=>push(type,detail);
  ...
  function report(){
    return {
      ...
      xrTrace:(()=>{try{return window.__ROOM_SCANNER_D13_XR_REPORT__?window.__ROOM_SCANNER_D13_XR_REPORT__():null}catch(e){return {error:String(e&&e.stack||e)}}})(),
      d13SourcePatch:{depthGate:window.__ROOM_SCANNER_D13_DEPTH_GATE_STATE__||null,photoGuardCount:window.__ROOM_SCANNER_D13_PHOTO_GUARD_COUNT__||0},
      previousTrace,
  ...
  function updateLiveTrace(){
    ...
    const lines=['ROOM SCANNER V11-D13  |  '+BUILD];
    try{const xr=window.__ROOM_SCANNER_D13_XR_REPORT__&&window.__ROOM_SCANNER_D13_XR_REPORT__();for(const x of (xr&&xr.events||[]).slice(-8))lines.push('XR '+compactEventLine(x))}catch(_){}
  ...
  function ensurePanel(){
    ...
    panel.id='depthV11D13Diag';
    badge=document.getElementById('v11D13StaticBanner');
    ...
    panel.querySelector('#depthV11Wasm').onclick=()=>{try{sessionStorage.setItem('roomScannerV11D13ForceWasm','1')}catch(_){};try{if(typeof S!=='undefined'&&S&&S.depthAI)S.depthAI.forceWasm=true}catch(_){};push('manual-force-wasm',{applied:true});status.textContent='WASM forzato per la prossima inizializzazione Depth';};
    ...
    panel.querySelector('#depthV11Prev').onclick=()=>{ta.value=previousTrace?JSON.stringify(previousTrace,null,2):'Nessun log D13 precedente salvato';ta.style.display='block'};
