/*
 * Room Scanner V20.3 - dense Gaussian-surfel map worker.
 *
 * Design goal:
...
let sessionId=null,totalPoints=0,droppedPoints=0,batches=0,lastPrune=0;
...
function init(m){
  ...
  CFG.budget=Math.max(30000,Math.min(90000,Math.round(requested*1.38)));
  cells=new Map();planeCells=new Map();totalPoints=0;droppedPoints=0;batches=0;
  postMessage({type:'ready',sessionId,gaussianSurfel:true,voxelM:CFG.voxel,budget:CFG.budget});
}
...
function ingest(m){
  ...
  if(batches%5===0)postMessage({type:'stats',cells:cells.size,planeCells:planeCells.size,totalPoints,droppedPoints,batches,budget:CFG.budget});
}
...
function principalScales(cov){
  // Full eigendecomposition is unnecessary online. Trace plus the smallest
  // diagonal/cross-term proxy gives stable anisotropy for coverage and export.
  const sx=Math.sqrt(Math.max(1e-6,cov[0])),sy=Math.sqrt(Math.max(1e-6,cov[3])),sz=Math.sqrt(Math.max(1e-6,cov[5])),s=[sx,sy,sz].sort((a,b)=>b-a);return {tangentScale:clamp(s[0]*2.4,.015,.18),secondScale:clamp(s[1]*2.4,.012,.15),normalScale:clamp(s[2]*2.0,.006,.075)};
}
...
function snapshot(m){
  ...
  postMessage({type:'snapshot',requestId:m.requestId||null,tiles:arr,stats:{cells:cells.size,planeCells:planeCells.size,predictedTargets:predicted.length,totalPoints,droppedPoints,batches,confirmedGaussians:real.filter(s=>s.gaussian?.confirmed).length}});
}
...
function reset(){cells.clear();planeCells.clear();totalPoints=0;droppedPoints=0;batches=0;}
