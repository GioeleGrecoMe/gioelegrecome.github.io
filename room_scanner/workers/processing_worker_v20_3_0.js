/*
 * Room Scanner V20.3 - post-XR metric + Deep multi-view fusion worker.
 *
...
import {buildAcousticReadyModel,clusterResidualObjects,decimateSurfels,fitStructuralPlanes} from '../js/reconstruction_v20_3_0.js';
...
async function run({sessionId,mode='safe',memoryBudgetMB=320}){
  ...
  const byKind=groupRecords(records),depthBlobs=blobRecords.filter(b=>b.kind==='depth-points'),frameBlobs=blobRecords.filter(b=>b.kind==='frame-jpeg');
  ...
  const maxSurfels=Math.max(26000,Math.min(105000,Math.floor(memoryBudgetMB*225)));
  ...
  stage(5,`Caricati ${depthBlobs.length} batch metrici e ${frameBlobs.length} fotografie`);

  const accumulator=new GaussianAccumulator({maxCells:maxSurfels,voxel:.042});
  for(let i=0;i<depthBlobs.length;i++){
    ...
    if(i%4===0)stage(5+24*(i+1)/Math.max(1,depthBlobs.length),`Fusione metrica ${i+1}/${depthBlobs.length}`);
  }
  ...
  let points=accumulator.toSurfels();stage(30,`${points.length} Gaussian surfel metrici prima di Deep`);await saveCheckpoint(repo,sessionId,'metric-fusion',{surfelCount:points.length,sourceBatches:depthBlobs.length,registration,skippedUnregisteredBatches});

  let deepSummary={status:'not-requested'};
  if(mode==='deep'&&frameBlobs.length){
    try{deepSummary=await mergeDeepFrames({frameBlobs,frameRecords:byKind.get('frame-meta')||[],accumulator,registration,referenceSegment,memoryBudgetMB,progressBase:31,progressSpan:34});points=accumulator.toSurfels({dropUnconfirmedDeep:true});stage(66,`Deep multi-view: ${points.length} Gaussian surfel, ${deepSummary.validFrames}/${deepSummary.selectedFrames} frame calibrati`);await saveCheckpoint(repo,sessionId,'deep-fusion',deepSummary);}
    catch(error){deepSummary={status:'failed',message:error.message};post('warning',66,`Depth Anything non disponibile; mantengo la mappa WebXR: ${error.message}`);}
  }

  check();stage(70,'Decimazione adattativa senza distruggere bordi e oggetti');
  points=decimateSurfels(points,{maxPoints:Math.min(maxSurfels,90000)});await saveCheckpoint(repo,sessionId,'decimation',{surfelCount:points.length});check();
  stage(77,'Fit strutturale sopra la mappa densa (la mappa originale resta nel modello)');const fit=fitStructuralPlanes(points);await saveCheckpoint(repo,sessionId,'structural-fit',{stats:fit.stats,planes:fit.planes});check();
  stage(87,'Raggruppo i residui persistenti in oggetti RGB');const objects=clusterResidualObjects(fit.residual,{maxPoints:Math.min(26000,Math.floor(memoryBudgetMB*70))});check();
  ...
  stage(94,'Creo modello 3D + superfici acusticamente indirizzabili');const model=buildAcousticReadyModel({session,points,fit,objects,markpoints,frames,chirps,audio,registration,diagnostics:events});
  ...
  model.format='ROOMSCAN-MODEL-20.3';model.geometry.gaussianCount=points.filter(p=>p.gaussian).length;model.geometry.gaussianEncoding='surfel.mean+covariance6+scale+normal+rgb+confidence+provenance';model.processing={...(model.processing||{}),deepFusion:deepSummary};
...
class GaussianAccumulator{
  ...
  _cellFor(p,source){
    ...
    if(source==='deep'){const near=this.nearest(p,.085);if(near)return near;}
  ...
  add(p){
    ...
    if(p.source==='deep'&&found.cell&&found.distance>.025){const mean=found.mean||[c.p[0]/c.w,c.p[1]/c.w,c.p[2]/c.w],nn=norm3(c.n),planeResidual=Math.abs(dot3(nn,sub3(p.position,mean)));const tol=.035+.018*Math.max(0,p.depthM||dist3(p.position,p.viewOrigin||p.position));if(c.sources.xr>=2&&planeResidual>tol)return false;if(c.sources.deep>=2&&found.distance>.095)return false;}
...
async function mergeDeepFrames({frameBlobs,frameRecords,accumulator,registration,referenceSegment,memoryBudgetMB,progressBase,progressSpan}){
  ...
  const maxFrames=Math.max(42,Math.min(144,Math.round(memoryBudgetMB*.34))),selected=selectDeepFrames(candidates,maxFrames);if(!selected.length)throw new Error('Nessuna fotografia con posa metrica valida');
  ...
  for(let i=0;i<selected.length;i++){
    check();const f=selected[i],result=await workerRequest(worker,{type:'process',requestId:`deep-${i}`,frameId:f.frameId,blob:f.blobRecord.blob,inputSize:518},'result',120000);inferenceTimes.push(result.inferenceMs||0);const depth=new Float32Array(result.depth),rgb=new Uint8Array(result.rgb),anchors=accumulator.toCalibrationPoints({max:7200}),cal=calibrateDepthRobust(depth,result.width,result.height,f.meta,anchors,temporalPrior);
    if(cal.valid){
      ...
      const dense=reprojectDeepDense(depth,rgb,result.width,result.height,f.meta,cal,f.frameId,{stride:selected.length>110?9:(selected.length>75?8:7)});let accepted=0;for(const p of dense){if(accumulator.add(p))accepted++;}acceptedPoints+=accepted;rejectedPoints+=dense.length-accepted;
...
function scaleProxy(cov){const a=[Math.sqrt(Math.max(1e-6,cov[0])),Math.sqrt(Math.max(1e-6,cov[3])),Math.sqrt(Math.max(1e-6,cov[5]))].sort((x,y)=>y-x);return [clamp(a[0]*2.3,.012,.16),clamp(a[1]*2.3,.010,.14),clamp(a[2]*2.0,.005,.065)];}
