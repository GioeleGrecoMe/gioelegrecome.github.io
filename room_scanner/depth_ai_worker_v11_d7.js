...
const V11_WORKER_BUILD = 'room-scanner-v11-d7-depth-worker-boottrace-2026-08-16';
...
// D7 critical fix: capture messages that arrive while this shim is still fetching and
...
function failWorker(error) {
  ...
  // D7: even bootstrap failures must answer requests that arrived before the handler existed.
...
async function boot() {
  try {
    ...
    // D7: WebGPU diagnostics must never block loading the Depth worker/model.
    ...
    // only the code payload; self.location remains depth_ai_worker_v11_d7.js, so same-directory
    // relative asset resolution remains correct.
    const blob = new Blob([patched.src + `\n//# sourceURL=depth_ai_worker_v11_d7_runtime.js\n`], { type: 'text/javascript' });
