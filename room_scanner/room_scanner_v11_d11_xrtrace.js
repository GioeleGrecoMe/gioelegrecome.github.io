(() => {
  ...
  // D11_XR_TRACE: loaded before the V10 application scripts. This file is diagnostics-only:
  // it records WebXR lifecycle and user-activation state without changing XR options or rendering.
  const BUILD = 'room-scanner-v11-d11-xrtrace-2026-08-16';
  const STORE = 'roomScannerV11D11XRTrace';
  const events = [];
  ...
  function stackText() {
    try { return String(new Error('D11 XR call stack').stack || '').split('\n').slice(1, 8).join('\n'); }
    catch (_) { return ''; }
  }
  ...
  function push(type, detail = {}) {
    ...
    try {
      window.__DEPTH_V11_D11_PUSH__ && window.__DEPTH_V11_D11_PUSH__('XR ' + type, item.detail);
    } catch (_) {}
  }
  ...
  function installXRHook() {
    ...
    if (xr.requestSession && xr.requestSession.__roomScannerD11Wrapped) {
    ...
    try { Object.defineProperty(wrapped, '__roomScannerD11Wrapped', { value: true }); } catch (_) {}
  ...
  window.__ROOM_SCANNER_D11_XR_REPORT__ = () => ({
    schema: 'room-scanner-v11-d11-xrtrace',
    build: BUILD,
