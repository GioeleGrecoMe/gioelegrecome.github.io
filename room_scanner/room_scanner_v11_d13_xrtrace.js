(() => {
  ...
  // D13_XR_TRACE: loaded before the V10 application scripts. This file is diagnostics-only:
  // it records WebXR lifecycle and user-activation state without changing XR options or rendering.
  const BUILD = 'room-scanner-v11-d13-xrtrace-2026-08-16';
  const STORE = 'roomScannerV11D13XRTrace';
  const events = [];
  ...
  function stackText() {
    try { return String(new Error('D13 XR call stack').stack || '').split('\n').slice(1, 8).join('\n'); }
    catch (_) { return ''; }
  }
  ...
  function push(type, detail = {}) {
    ...
    try {
      window.__DEPTH_V11_D13_PUSH__ && window.__DEPTH_V11_D13_PUSH__('XR ' + type, item.detail);
    } catch (_) {}
  }
  ...
  function installXRHook() {
    ...
    if (xr.requestSession && xr.requestSession.__roomScannerD13Wrapped) {
    ...
    const wrapped = async function(mode, options) {
      ...
      try {
        ...
        // D13 source patch keeps CFG.depthAIEnabled=false while the V10 startup path
        ...
        try {
          if (typeof window.__ROOM_SCANNER_D13_RESTORE_DEPTH__ === 'function') {
            push('xr-depth-restore-requested', { reason: 'xr-session-created' });
            window.__ROOM_SCANNER_D13_RESTORE_DEPTH__('xr-session-created');
          }
        } catch (restoreError) {
    ...
    try { Object.defineProperty(wrapped, '__roomScannerD13Wrapped', { value: true }); } catch (_) {}
  ...
  window.__ROOM_SCANNER_D13_XR_REPORT__ = () => ({
    schema: 'room-scanner-v11-d13-xrtrace',
    build: BUILD,
