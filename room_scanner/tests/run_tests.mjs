...
await test('static_adaptive_grid_and_no_wall_marking',async()=>{const html=await readFile(path.join(root,'room_scanner_v12.html'),'utf8'),worker=await readFile(path.join(root,'workers/map_worker_v20_2_0.js'),'utf8'),app=await readFile(path.join(root,'js/app_v20_2_0.js'),'utf8'),grid=await readFile(path.join(root,'js/grid_v20_2_0.js'),'utf8');assert.ok(html.includes('grid-overlay'));assert.ok(html.includes('Pinna al reticolo'));assert.ok(html.includes('markpoint-action'));assert.ok(!/Segna parete|Completa vano|angolo parete/i.test(html));assert.ok(worker.includes("surfaceType==='object'"));assert.ok(worker.includes('stableDepthGrid'));assert.ok(worker.includes('planeCells=next'));assert.ok(worker.includes('droppedPoints'));assert.ok(app.includes('recoverMismatchedShell'));assert.ok(app.includes('ui-control-missing'));assert.ok(app.includes('downloadPreparedFile({close:false})'));assert.ok(grid.includes('_drawReticle'));});
