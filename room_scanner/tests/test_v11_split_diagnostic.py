...
for token in [
    "const BUILD='v11.0.4-multiframe-recovery'",
    "navigator.mediaDevices.getUserMedia",
    "new Worker(`./depth_ai_worker.js?rsbuild=${REV}`)",
    "modelWasmLocal:'./models/depth_anything_v2_small_q4.onnx'",
    "forceWasm:true",
    "inputSize:392",
    "function scaleToShortSide720(sw,sh)",
    "short>720?720/short:1",
    "function freezeCameraFrame()",
    "Scatta frame (lato corto ≤ 720 px)",
    "id=\"deepLoad\"",
    "async function loadDepthFromUI()",
    "setTimeout(()=>loadDepthFromUI(),40)",
    "await workerRequest('smoke')",
    "await workerRequest('infer'",
    "async function inferDepthWithRecovery(frame)",
    "if(depthReady&&worker&&depthInfo)return depthInfo",
