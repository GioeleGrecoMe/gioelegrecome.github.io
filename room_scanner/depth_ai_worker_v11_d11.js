...
const V11_WORKER_BUILD = 'room-scanner-v11-d11-depth-worker-boottrace-2026-08-16';
...
// D11 critical fix: capture messages that arrive while this shim is still fetching and
...
function failWorker(error) {
  ...
  // D11: even bootstrap failures must answer requests that arrived before the handler existed.
...
function augmentDepthDiagnostics(src, changes) {
  ...
  // D11 ORT ASSET FIX for an already-instrumented public worker.
  if (!src.includes('DEPTHAI_D11_ORT_ASSET_FIX') && !src.includes('const d11OrtDist =')) {
    const a="  ortApi.env.wasm.simd = true;\n";
    const b="  ortApi.env.wasm.simd = true; // DEPTHAI_D11_ORT_ASSET_FIX\n  const d11OrtDist = `https://cdn.jsdelivr.net/npm/onnxruntime-web@${runtimeVersion}/dist/`;\n  const d11WasmStem = wantsWebGPU ? 'ort-wasm-simd-threaded.asyncify' : 'ort-wasm-simd-threaded';\n  ortApi.env.wasm.wasmPaths = {mjs:`${d11OrtDist}${d11WasmStem}.mjs`,wasm:`${d11OrtDist}${d11WasmStem}.wasm`};\n  depthDebug('runtime-assets-pinned',{runtimeSource,runtimeVersion,wantsWebGPU,mjs:ortApi.env.wasm.wasmPaths.mjs,wasm:ortApi.env.wasm.wasmPaths.wasm});\n";
    if(src.includes(a))src=src.replace(a,b,1);else throw new Error('D11 ORT asset fix: wasm.simd anchor not found');
...
function patchDepthWorkerSource(source) {
  ...
  if (!src.includes('DEPTHAI_RUNTIME_ORIGIN_FIX')) {
    ...
    changes.push('D11 explicit ORT mjs/wasm asset pin');
  }
...
`;
  ...
  src = replaceOnce(
    ...
    "  ortApi.env.wasm.simd = true;\n" +
    "  // DEPTHAI_D11_ORT_ASSET_FIX: single-injection marker; the D8 trace proved the local vendor directory is missing the Emscripten .mjs artifact.\n" +
  ...
  // D11 LOCAL-FIRST: prefer the flat same-origin model uploaded next to this worker/page.
...
async function boot() {
  try {
    ...
    // D11: WebGPU diagnostics must never block loading the Depth worker/model.
    ...
    // only the code payload; self.location remains depth_ai_worker_v11_d11.js, so same-directory
    // relative asset resolution remains correct.
    const blob = new Blob([patched.src + `\n//# sourceURL=depth_ai_worker_v11_d11_runtime.js\n`], { type: 'text/javascript' });
