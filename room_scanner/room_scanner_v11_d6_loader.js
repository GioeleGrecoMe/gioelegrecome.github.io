(() => {
  ...
  const BUILD = 'room-scanner-v11-d6-safe-external-loader-2026-08-16-a';
  const V10_URL = './room_scanner_v10.html';
  const WORKER_URL = './depth_ai_worker_v11_d6.js';
  const DIAG_URL = './room_scanner_v11_d6_diag.js?v=d6-20260816-a';
  const events = [];
  ...
  function log(stage, detail = {}) {
    ...
    events.push(item);
    try {
      localStorage.setItem('roomScannerV11D6LoaderTrace', JSON.stringify({ build: BUILD, href: location.href, events: events.slice(-80) }));
    } catch (_) {}
    const live = document.getElementById('v11LoaderLive');
    if (live) {
      live.textContent = 'ROOM SCANNER V11-D6 / LOADER TRACE\n' + events.slice(-24).map(x => `[${x.t_ms}ms] ${x.stage} ${safeJson(x.detail)}`).join('\n');
    }
  }
  ...
  async function boot() {
    try {
      ...
      const sourceUrl = `${V10_URL}?v11d6_source=${encodeURIComponent(BUILD)}&ts=${Date.now()}`;
      ...
      html = replaceExactlyOnce(html, workerAnchor, "depthAIWorker:'./depth_ai_worker_v11_d6.js'", 'Depth worker URL');
      ...
      if (html.includes(oldVisibleLabel)) {
        html = html.replace(oldVisibleLabel, 'ROOM SCANNER V11-D6 · DEPTH TRACE + RAW');
        log('visible-version-label-rewritten', { from: oldVisibleLabel, to: 'ROOM SCANNER V11-D6 · DEPTH TRACE + RAW' });
      } else {
      ...
      if (!html.includes("depthAIWorker:'./depth_ai_worker_v11_d6.js'")) throw new Error('Worker D6 non presente dopo trasformazione');
      if (!html.includes('ROOM SCANNER V11-D6')) throw new Error('Marker V11-D6 non presente dopo trasformazione');
      if (!html.includes('room_scanner_v11_d6_diag.js')) throw new Error('Diagnostica D6 non presente dopo trasformazione');
