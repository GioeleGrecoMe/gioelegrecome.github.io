(() => {
  ...
  const BUILD = 'room-scanner-v11-d8-safe-external-loader-2026-08-16-queuefix';
  const V10_URL = './room_scanner_v10.html';
  const WORKER_URL = './depth_ai_worker_v11_d8.js';
  const DIAG_URL = './room_scanner_v11_d8_diag.js?v=d8-queuefix-20260816';
  const events = [];
  ...
  function log(stage, detail = {}) {
    ...
    events.push(item);
    try {
      localStorage.setItem('roomScannerV11D8LoaderTrace', JSON.stringify({ build: BUILD, href: location.href, events: events.slice(-80) }));
    } catch (_) {}
    const live = document.getElementById('v11LoaderLive');
    if (live) {
      live.textContent = 'ROOM SCANNER V11-D8 / LOADER TRACE\n' + events.slice(-24).map(x => `[${x.t_ms}ms] ${x.stage} ${safeJson(x.detail)}`).join('\n');
    }
  }
  ...
  async function boot() {
    try {
      ...
      const sourceUrl = `${V10_URL}?v11d8_source=${encodeURIComponent(BUILD)}&ts=${Date.now()}`;
      ...
      html = replaceExactlyOnce(html, workerAnchor, "depthAIWorker:'./depth_ai_worker_v11_d8.js'", 'Depth worker URL');
      ...
      if (html.includes(oldVisibleLabel)) {
        html = html.replace(oldVisibleLabel, 'ROOM SCANNER V11-D8 · DEPTH TRACE + RAW');
        log('visible-version-label-rewritten', { from: oldVisibleLabel, to: 'ROOM SCANNER V11-D8 · DEPTH TRACE + RAW' });
      } else {
      ...
      const banner = '<div id="v11D8StaticBanner" style="position:fixed;z-index:2147483647;top:0;left:0;right:0;background:#ffcf33;color:#111;padding:8px 10px;font:900 14px/1.2 system-ui,-apple-system,sans-serif;text-align:center;box-shadow:0 2px 8px #0008">ROOM SCANNER V11-D8 • DEPTH TRACE ACTIVE • BUILD 2026-08-16-QUEUEFIX</div>';
      html = html.replace(bodyMatch[0], bodyMatch[0] + banner);
      log('static-d8-banner-injected');
      ...
      if (!html.includes("depthAIWorker:'./depth_ai_worker_v11_d8.js'")) throw new Error('Worker D8 non presente dopo trasformazione');
      if (!html.includes('ROOM SCANNER V11-D8')) throw new Error('Marker V11-D8 non presente dopo trasformazione');
      if (!html.includes('room_scanner_v11_d8_diag.js')) throw new Error('Diagnostica D8 non presente dopo trasformazione');
