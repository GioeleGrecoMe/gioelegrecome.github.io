...
export function evaluateLiveCandidate({baselineStats=null,candidateStats=null,baselineSnapshot=null,candidateSnapshot=null,options={}}={}){
  const cfg={maxReprojectionPx:Number(options.maxReprojectionPx??3.2),maxReprojectionGrowth:Number(options.maxReprojectionGrowth??1.28),reprojectionSlackPx:Number(options.reprojectionSlackPx??.18),maxCommonTranslationJump:Number(options.maxCommonTranslationJump??.11),maxCommonRotationJumpRad:Number(options.maxCommonRotationJumpRad??(4*Math.PI/180)),maxMeanTranslationJump:Number(options.maxMeanTranslationJump??.045),maxMeanRotationJumpRad:Number(options.maxMeanRotationJumpRad??(1.7*Math.PI/180)),maxDepthErrorGrowth:Number(options.maxDepthErrorGrowth??1.55),depthErrorSlack:Number(options.depthErrorSlack??.025),maxRejectedEdgeGrowth:Number(options.maxRejectedEdgeGrowth??3),minObservations:Number(options.minObservations??10),minLandmarks:Number(options.minLandmarks??4)};
  ...
  if(pose.commonFrames>=2){if(pose.maxTranslation>cfg.maxCommonTranslationJump)hard.push('pose-translation-jump');if(pose.maxRotationRad>cfg.maxCommonRotationJumpRad)hard.push('pose-rotation-jump');if(pose.meanTranslation>cfg.maxMeanTranslationJump)warnings.push('pose-mean-translation-large');if(pose.meanRotationRad>cfg.maxMeanRotationJumpRad)warnings.push('pose-mean-rotation-large');}
